    ...
    class GoSendEx {
        ...
        function process_order_submit_to_gosendex($order) {
			...
			if ($order_status !== 'failed') {
				...
				$order->add_order_note("[GoSendEx] Not submitted to GoSendEx because of Failed Order Status.");
			}
        }
        ...
        public function order_status_processing($order_id) {
            ...
            foreach ($_order as $order_item_id => $order_product_detail) {
                $gseBasePath = $order_item_id;
                if (isset($settings['gosendex_group_order_files'])) {
                    if ($settings['gosendex_group_order_files']) {
                        $gseBasePath = "";
                    }
                }
            ...
            $gse_order_xml->Filename = "WP_" . $this->_get_order_number($order_id) . ".xml";
            
            $gse_file_set[] = $gse_order_xml;
            if (!empty($gse_file_set)) {
                $gse_meta_data = array();
                $gse_meta_data[] = $this->_generate_meta_data_object("Order Id", $order_id);
                $data = array('files' => json_encode($gse_file_set), 'fileName' => "WP_" . $this->_get_order_number($order_id), 'metaData' => json_encode($gse_meta_data));
        ...
        public function get_gosendex_api_url(): string
        {
        ...
        public function generate_order_xml($order_id) {            
            ...
            foreach ($order_items as $order_item_id => $order_product_detail) {
                ...
                $order_line->addChild("Name",               htmlspecialchars($order_product_detail['name']));
				...
				if (isset($order_product_detail['_udraw_jpg_pages'])) {
                    if (is_array($order_product_detail['_udraw_jpg_pages']) && count($order_product_detail['_udraw_jpg_pages']) > 0) {
						...
						for($z = 0; $z < $attached_jpgs; $z++) {
							$udraw_jpg_file = $udraw_jpg_files->addChild("ProductionJPGFile", (isset($order_product_detail['_udraw_jpg_pages'][$z])) ? $order_product_detail['_udraw_jpg_pages'][$z] : "");
						}
					}
				}
        ...
        private function _get_order_number($order_id) {
            // Check to see if custom order number is set from a different plugin.  If so, we will override
            // instead of using WooCommerce default order id.
