...
class uDraw_Clipart_Table extends WP_List_Table {
    ...
    function column_category($item) {
        echo '<select id="select-clipart-category-id-'. uDrawUtil::esc_string($item["access_key"]) .'" class="select-clipart-category" style="width: 150px;"';
        if (!current_user_can('edit_udraw_clipart_upload')) {
            echo ' disabled';
        }
        echo '>';
        ...
        <script>
            var categoryID = '<?php echo uDrawUtil::esc_string($item["category"]) ?>';
            if (categoryID !== '' && categoryID != null) {
                jQuery('#select-clipart-category-id-<?php echo uDrawUtil::esc_string($item["access_key"])?> option[value="'+categoryID+'"]').prop('selected', true);
            }
        </script>
        <?php
    }
    ...
    function prepare_items()
    {
        ...
        $orderby = (isset($_REQUEST['orderby']) && in_array($_REQUEST['orderby'], array_keys($this->get_sortable_columns()))) ? uDrawUtil::sanitize_variables($_REQUEST['orderby']) : 'date';
        ...
        $filter = isset($_GET['filter']) ? uDrawUtil::sanitize_variables($_GET['filter']) : '\'%%\'';
    ...
    function buildCategorySelectOptions ($parent_id) {
        foreach ($this->_clipart_category_array as $category) {
            if ($category->parent_id == $parent_id) {
                echo '<option value="'. uDrawUtil::esc_string($category->ID) .'">'.stripslashes($this->buildClipartCategoryPath($category->ID, "")).'</option>';
