...
class uDraw_Public_Templates_Table extends WP_List_Table {
    ...
    function extra_tablenav( $which ) {
        if ( 'top' == $which ) {
            ...
            if (isset($_GET["category_id"])) {
                $category_id = intval(uDrawUtil::sanitize_variables($_GET["category_id"]));
            }
            ...
            foreach($this->_categories as $category) {
                ...
                echo "\t<option value='" . uDrawUtil::esc_string($category->Id) . "' ". uDrawUtil::esc_string($selected) . ">". uDrawUtil::esc_string($category->Name) ."</option>\n";
                if (count($category->Children) > 0) {
                    foreach ($category->Children as $child_cateogry) {
                        $selected = "";
                        if ($child_cateogry->Id == $category_id) { $selected = "selected=\"selected\""; }
                        echo "\t<option value='" . uDrawUtil::esc_string($child_cateogry->Id) . "' ". uDrawUtil::esc_string($selected) . ">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;". uDrawUtil::esc_string($child_cateogry->Name) ."</option>\n";
                    }
                }
            }
            echo "</select>\n";
    ...
    function prepare_items()
    {
        ...
        if (isset($_GET["category_id"])) {
            $category_id = uDrawUtil::sanitize_variables($_GET["category_id"]);
        }
        ...
        if (isset($_GET['orderby'])) {
            if ($_GET['order'] == "asc") {
                $sortedArray = $this->array_sort($this->items, uDrawUtil::sanitize_variables($_GET['orderby']), SORT_ASC);
                $this->items = $sortedArray;
            } else {
                $sortedArray = $this->array_sort($this->items, uDrawUtil::sanitize_variables($_GET['orderby']), SORT_DESC);
                $this->items = $sortedArray;
            }            
        }
