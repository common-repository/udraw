...
function column_category($item)
    {   
                    ...
                    if (is_array($categoryResult)) {
                        if (count($categoryResult) > 0) {
                            ...
                            if (current_user_can('edit_udraw_templates')) {
                                echo '<a href="#" id="remove-category-from-'. uDrawUtil::esc_string($item['id']) .'" style="text-decoration: none;" onclick="javascript: removeCategoryFrom('. uDrawUtil::esc_string($item['id']) .','. uDrawUtil::esc_string($category) .')">x</a>';
                            }
        ...
        if (current_user_can('edit_udraw_templates')) {
            echo '<div>';
            echo '<select id="select-category-for-'. uDrawUtil::esc_string($item['id']) .'" class="add-template-category" style="width: 135px; display: inline-block;">';
    ...
    {        
        $views = array();
        $current = ( !empty($_REQUEST['udraw_view']) ? uDrawUtil::sanitize_variables($_REQUEST['udraw_view']) : 'all');
    ...
    function prepare_items()
    {
        ...
        $orderby = (isset($_REQUEST['orderby']) && in_array($_REQUEST['orderby'], array_keys($this->get_sortable_columns()))) ? uDrawUtil::sanitize_variables($_REQUEST['orderby']) : 'create_date';
        $order = (isset($_REQUEST['order']) && in_array($_REQUEST['order'], array('asc', 'desc'))) ? uDrawUtil::sanitize_variables($_REQUEST['order']) : 'desc';
        $search = isset($_REQUEST['s']) ? '%%'. uDrawUtil::sanitize_variables($_REQUEST['s']) . '%%' : '%%';
        $category = isset($_GET['filter']) ? uDrawUtil::sanitize_variables($_GET['filter']) : '%%';
        ...
        if (isset($_GET['udraw_view'])) {
            $filteredView = uDrawUtil::sanitize_variables($_GET['udraw_view']);
        ...
        for ($x = 0; $x < count($this->_templates_category_list); $x++) {
            if ($this->_templates_category_list[$x]->ID == $id) {
                ...
                if ($parentID == 0) {
                    ...
                    echo uDrawUtil::esc_string($newPath);
                } else {
    ...
    function buildCategorySelect($parentId, $parentName) {
        for ($x = 0; $x < count($this->_templates_category_list); $x++) { 
            if ($this->_templates_category_list[$x]->parent_id == $parentId) {
                ...
                echo "<option value='". uDrawUtil::esc_string($this->_templates_category_list[$x]->ID) ."'>";
                echo uDrawUtil::esc_string($parentName);
                if (strlen($parentName) > 0) {
                    echo " >> ";
                }
                
                echo uDrawUtil::esc_string($category_name);
                echo '</option>';
