...
class uDraw_Text_Templates extends WP_List_Table {    
    ...
    function column_category($item) {
        echo '<select id="select-text-template-category-id-'. uDrawUtil::esc_string($item["public_key"]) .'" class="select-text-template-category" style="width: 150px;"';
        if (!current_user_can('edit_udraw_templates')) {
            echo ' disabled';
        }
        echo '>';
        ...
        <script>
            ...
            if (categoryID !== '' && categoryID != null) {
                jQuery('#select-text-template-category-id-<?php echo uDrawUtil::esc_string($item["public_key"])?> option[value="'+categoryID+'"]').prop('selected', true);
            }
        </script>
        <?php
    }
    ...
    function buildCategorySelectOptions ($parent_id) {
        foreach ($this->_text_templates_category_array as $category) {
            if ($category->parent_id == $parent_id) {
                echo '<option value="'. uDrawUtil::esc_string($category->ID) .'">'.stripslashes($this->buildTextTemplatesCategoryPath($category->ID, "")).'</option>';
