    ...
    class uDrawConnect {
            ...
            for ($x = 0; $x < count($item_keys); $x++) {
                ...
                if (isset($udraw_data['udraw_product_data']) && strlen($udraw_data['udraw_product_data']) > 0) {         
                    ...
                    if (strlen($udraw_data['udraw_product_data']) < 100) {
                        $uDrawOrder->data = $uDrawUtil->get_web_contents(UDRAW_STORAGE_URL . $udraw_data['udraw_product_data']);
                    }
            ...
            foreach ($uDrawOrdersRequest as $uDrawRequest) {
                ...
                if ($uDrawRequest->type == 'blocks') {
                    ...
                    if (strlen($uDrawRequest->block_id) > 10) {
                        ...
                        $customID = isset($_auth_object->CustomID) ? $_auth_object->CustomID : '';
                        $data = array(
                        ...
                        if (!is_null($response)) {
                            if ($response->status == "wait") {
                                ...
                                if (strlen($response->src) > 0) {
                                    ...
                                    $data = array(
                                        'pdfDocument' => str_replace('localhost', 'udraw-wordpress.ngrok.io', $response->src),
        ...
        private function base64_to_image($base64_string, $output_file) {
            ...
            if (is_file($base64_string)) {
                $ifp = fopen($output_file, "wb");
                $base64_string = str_replace(UDRAW_STORAGE_DIR, UDRAW_STORAGE_URL, $base64_string);
                $data = $uDrawUtil->get_web_contents($base64_string);
                fwrite($ifp, $data); 
                fclose($ifp);
            }
        ...
        private function update_thirdparty_systems($order_id) {
            ...
            foreach ($order_item_keys as $x => $value) {
                ...
                $_item_object->Thumbnail = ""; // This gets auto generated by GoEpower service.
