...
if (!class_exists('uDrawDesignHandler')) {
    class uDrawDesignHandler extends uDrawAjaxBase {
                ...
                if (file_exists(UDRAW_FONTS_DIR . $_REQUEST['font_name'] . $_REQUEST['font_type']) || file_exists(UDRAW_FONTS_DIR . $_REQUEST['font_name'] . strtoupper($_REQUEST['font_type'])) ) {
                    if (current_user_can('delete_udraw_fonts')) {
                        if (file_exists(UDRAW_FONTS_DIR . $_REQUEST['font_name'] . $_REQUEST['font_type'])) {
                            unlink(UDRAW_FONTS_DIR . uDrawUtil::sanitize_variables($_REQUEST['font_name']) . uDrawUtil::sanitize_variables($_REQUEST['font_type']));
                            return $this->sendResponse(true);
                        } else {
                            unlink(UDRAW_FONTS_DIR . uDrawUtil::sanitize_variables($_REQUEST['font_name']) . strtoupper(uDrawUtil::sanitize_variables($_REQUEST['font_type'])));
                            return $this->sendResponse(true);
                        }                        
                    }
                }
        ...
        function upload() {
            $assetPath = uDrawUtil::sanitize_variables($_REQUEST['assetPath']);
        ...
        function local_images() {
            $localImagePath = uDrawUtil::sanitize_variables($_REQUEST['localImagePath']);
        ...
        function remove_image() {
            ...
            $file = uDrawUtil::sanitize_variables($_REQUEST['image_file']);
            $_asset_path = "_UNSET_";
            if (is_user_logged_in()) {
                $_asset_path = UDRAW_STORAGE_DIR . wp_get_current_user()->user_login . '/assets/';
            } else {
                if (isset($_REQUEST['session_id'])) {
                    $session_id = uDrawUtil::sanitize_variables($_REQUEST['session_id']);
        ...
        function local_patterns() {
            $localPatternsPath = uDrawUtil::sanitize_variables($_REQUEST['localPatternsPath']);
            $this->__process_images($localPatternsPath, false);
            wp_die();
        }
        
        function upload_patterns() {
            $patternPath = uDrawUtil::sanitize_variables($_REQUEST['patternPath']);
        ...
        function export_preview_image () {
            ...
            $page_no = 1;
            $preview_data = '';
            $outputPath = uDrawUtil::sanitize_variables($_REQUEST['outputPath']);
            ...
            if (isset($_REQUEST['user_session_id']) && strlen($_REQUEST['user_session_id']) > 0) {
                $user_session_id = uDrawUtil::sanitize_variables($_REQUEST['user_session_id']);
            }
            if (isset($_REQUEST['page_no'])) {
                $page_no = uDrawUtil::sanitize_variables($_REQUEST['page_no']);
            }
            if (isset($_REQUEST['data'])) {
                $preview_data = uDrawUtil::sanitize_variables($_REQUEST['data']);
            }
        ...
        function save_page_data() {
            ...
            try {
                ...
                $pageNo = uDrawUtil::sanitize_variables($_REQUEST['pageNo']);
        ...
        function compile_design_data() {
            ...
            try {
                ...
                // Save Preview Image
                $previewData = uDrawUtil::sanitize_variables($_REQUEST['previewDoc']);
        ...
        public function create_translation_file ($language = '', $display_name = '') {
            if (isset($_REQUEST['language'])) {
                $language = uDrawUtil::sanitize_variables($_REQUEST['language']);
            }
            if (isset($_REQUEST['display_name'])) {
                $display_name = uDrawUtil::sanitize_variables($_REQUEST['display_name']);
            } else {
        ...
        public function retrieve_instagram() {
            ...
            $term = (isset($_REQUEST['term'])) ? uDrawUtil::sanitize_variables($_REQUEST['term']) : '';
            //If there is a search term
            if (strlen($term) > 0) {
                $data = json_decode($uDrawUtil->get_web_contents('https://api.instagram.com/v1/tags/'.$term.'/media/recent?access_token='.$access_token));
            } else {
                $data = json_decode($uDrawUtil->get_web_contents('https://api.instagram.com/v1/users/self/media/recent?access_token='.$access_token));
            }
        ...
        public function authenticate_flickr() {
            ...
            $oauth_callback_string = $uDrawUtil->get_web_contents('http://www.flickr.com/services/oauth/request_token?' . http_build_query($oAuth_params));
        ...
        public function flickr_access_token($oauth_token = '', $oauth_verifier = '', $oauth_token_secret = '') {
            ...
            if (isset($_GET['oauth_token'])) {
                $oauth_token = uDrawUtil::sanitize_variables($_GET['oauth_token']);
            }
            if (isset($_GET['oauth_verifier'])) {
                $oauth_verifier = uDrawUtil::sanitize_variables($_GET['oauth_verifier']);
            }
            ...
            $oAuth_params['oauth_signature'] = $sign_string;
            $user_string = $uDrawUtil->get_web_contents('http://www.flickr.com/services/oauth/access_token?' . http_build_query($oAuth_params));
        ...
        public function flickr_get ($oauth_token = '', $oauth_token_secret = '', $method = '', $user_id = '', $photoset_id = '') {
            ...
            if (isset($_REQUEST['oauth_token'])) {
                $oauth_token = uDrawUtil::sanitize_variables($_REQUEST['oauth_token']);
            }
            if (isset($_REQUEST['oauth_token_secret'])) {
                $oauth_token_secret = uDrawUtil::sanitize_variables($_REQUEST['oauth_token_secret']);
            }
            if (isset($_REQUEST['method'])) {
                $method = uDrawUtil::sanitize_variables($_REQUEST['method']);
            }
            if (isset($_REQUEST['user_id'])) {
                $user_id = uDrawUtil::sanitize_variables($_REQUEST['user_id']);
            }
            if (isset($_REQUEST['photoset_id'])) {
                $photoset_id = uDrawUtil::sanitize_variables($_REQUEST['photoset_id']);
            }
            $api_key = $udraw_settings['designer_flickr_client_id'];
            $secret = $udraw_settings['designer_flickr_secret_id'];
            $timestamp = time();
            ...
            echo json_encode($uDrawUtil->get_web_contents('https://api.flickr.com/services/rest?' . http_build_query($oAuth_params)));
        }
        
        public function create_product_xml_pages ($path_url = '', $xml_array = array()) {
            if (isset($_REQUEST['folder_path']) && isset($_REQUEST['xml_array'])) {
                $path_url = sanitize_url($_REQUEST['folder_path']);
        ...
        public function create_merged_xml ($path_url = '', $canvas_header = '', $pages_length = 1, $cart_path_url = '') {
            ...
            if (isset($_REQUEST['cart_folder_path'])) {
                $cart_path_url = sanitize_url($_REQUEST['cart_folder_path']);
            } else {
                $cart_path_url = $path_url;
            }
            if (isset($_REQUEST['pages_length'])) {
                $pages_length = uDrawUtil::sanitize_variables($_REQUEST['pages_length']);
            }
            $path = str_replace(UDRAW_STORAGE_URL, UDRAW_STORAGE_DIR, $path_url);
            ...
            if (count($files_array) > 0) {
                ...
                for ($j = 0; $j < $pages_length; $j++) {
                    $string .= $uDrawUtil->get_web_contents($path_url . $j . '.xml');
                }
                $string .= '</canvas>';
        ...
        function udraw_send_saved_design_link ($email = '', $design_url = '', $preview_url = '', $date = '', $product_id = 0) {
            ...
            if (isset($_REQUEST['design_url'])) {
                $design_url = sanitize_url($_REQUEST['design_url']);
            }
            if (isset($_REQUEST['preview_url'])) {
                $preview_url = sanitize_url($_REQUEST['preview_url']);
            }
        ...
        function convert_pdf($key = '', $type = '', $pdf_action = '', $file = '', $pageNo = 1) {
            if (isset($_REQUEST['key']) && isset($_REQUEST['type']) && isset($_REQUEST['action']) && isset($_REQUEST['document'])) {
                $key = uDrawUtil::sanitize_variables($_REQUEST['key']);
                $type = uDrawUtil::sanitize_variables($_REQUEST['type']);
                $pdf_action = uDrawUtil::sanitize_variables($_REQUEST['pdf_action']);
                $file = uDrawUtil::sanitize_variables($_REQUEST['document']);
            }
			
            $pageNo = (isset($_REQUEST['page'])) ? uDrawUtil::sanitize_variables($_REQUEST['page']) : 1;
            ...
            if ( $type == 'png' && $pdf_action == 'convert') {
				...
				$data = array(
                    'pdfDocument' => str_replace('localhost', 'udraw-wordpress.ngrok.io', $file),
					'key' => uDraw::get_udraw_activation_key(),
				);
				$headers = array(
                ...
                error_log(print_r(UDRAW_CONVERT_URL."key=".$key."&type=".$type."&action=".$pdf_action."&document=".urlencode($file).'&page='.$pageNo, true));
        ...
        public function export_image ($dataURL = '', $assetPath = '') {
            ...
            if (isset($_REQUEST['dataURL'])) {
                $dataURL = sanitize_url($_REQUEST['dataURL']);
            }
            if (strlen($dataURL) === 0) {
                $this->sendResponse(false);
            }
        ...
        private function __process_upload($assetPath) {
            ...
            $newFile = rand(1, 32) .'_'. preg_replace(array('~[^0-9.a-z]~i'),'-', $removeSpaces) . '.' . $fileExt;
            
            $accepted_file_list = ['pdf', 'eps', 'jpg', 'jpeg', 'png', 'svg', 'gif', 'tiff', 'tif'];
            ...
            for ($f = count($accepted_file_list) - 1; $f > 0; $f--) {
                ...
                if (!in_array($accepted_file_list[$f], ['pdf', 'eps', 'jpg', 'jpeg', 'png', 'svg', 'gif', 'tiff', 'tif'])) {
                    unset($accepted_file_list[$f]);
                }
            }
            // See if we can convert the file before dismissing it.
            if (($fileExt == 'pdf' || $fileExt == 'eps' || $fileExt == 'tif' || $fileExt == 'tiff') && in_array($fileExt, $accepted_file_list)) {
                if (strlen(uDraw::get_udraw_activation_key()) > 0) {
                    // Save EPS, PDF AND TIFF locally to the sysetm.
                    $uploaded_files = $uDrawUpload->handle_upload($_FILES['files'], $assetDir, $assetBaseURL . $assetPath, array('pdf' => 'application/pdf', 'eps' => 'application/postscript', 'tif' => 'image/tiff', 'tiff' => 'image/tiff') );
                    ...
                    if (is_array($uploaded_files)) {
                        if ( !key_exists('error', $uploaded_files[0]) ) {
                            ...
                            if ($fileExt == "eps") {
                                ...
                                $endpoint_action = "EPS2PNG";
                            } else if ($fileExt == "tiff" || $fileExt == "tif") {
                                $data = array(
                                    'imageDocument' => str_replace('localhost', 'udraw-wordpress.ngrok.io', $uploaded_files[0]['url']),
                                    'key' => uDraw::get_udraw_activation_key()
                                );
                                $endpoint_action = "TIFF2PNG";
                            } else {
                                $data = array(
                                    'pdfDocument' => str_replace('localhost', 'udraw-wordpress.ngrok.io', $uploaded_files[0]['url']),
                                    'key' => uDraw::get_udraw_activation_key()
                                );
                                $endpoint_action = "PDF2PNG";
                            }
                            ...
                            if ($udraw_convert_response->isSuccess) {
                                if (!is_array($udraw_convert_response->data)) {
                                    array_push($url_array, UDRAW_CONVERT_SERVER_URL . $udraw_convert_response->data);
                                } else {
                            ...
                            // Once new converted document is saved, we can remove the uploaded PDF document.
        ...
        {
            $uDraw = new uDraw();
            $localFontPath = uDrawUtil::sanitize_variables($_REQUEST['localFontPath']);
        ...
        private function __read_translation_file($file) {
            ...
            $uDrawUtil = new uDrawUtil();
            $file = str_replace(UDRAW_PLUGIN_DIR, UDRAW_PLUGIN_URL, $file);
            return json_decode($this->remove_utf8_bom($uDrawUtil->get_web_contents($file))); 
        }
        ...
        public function udraw_purchase_saved_design() {
            ...
            if (isset($_REQUEST['saved_access_key'])) {
                $saved_design_access_key = uDrawUtil::sanitize_variables($_REQUEST['saved_access_key']);
            }
