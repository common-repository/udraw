    ...
    class uDrawTemplates extends uDrawAjaxBase {                
        ...
        function create_template($response ="", $output_path = "", $template_id = 0, $template_name = '', $doc_width = 0, $doc_height = 0, $doc_pages = 1) {
            ...
            if (isset($_REQUEST['response'])) {
                $response = uDrawUtil::sanitize_variables($_REQUEST['response']);
            }
            ...
            if (isset($_REQUEST['template_name'])) {
                $template_name = uDrawUtil::sanitize_variables($_REQUEST['template_name']);
            }
            
            if (isset($_REQUEST['doc_width'])) {
                $doc_width = uDrawUtil::sanitize_variables($_REQUEST['doc_width']);
            }
            
            if (isset($_REQUEST['doc_height'])) {
                $doc_height = uDrawUtil::sanitize_variables($_REQUEST['doc_height']);
            }
            
            if (isset($_REQUEST['doc_pages'])) {
                $doc_pages = uDrawUtil::sanitize_variables($_REQUEST['doc_pages']);
            }
            ...
            if (!is_null($sourceTemplate->tags) && strlen($sourceTemplate->tags) > 0) {
                ...
                if ($wpdb->get_var("SHOW TABLES LIKE '$tags_table'")) {
                    ...
                    if (isset($_REQUEST['offset'])) {
                        $offset = uDrawUtil::sanitize_variables($_REQUEST['offset']);
                    }
        ...
        function update_template_tags($template_id = 0, $tags = "") {
            ...
            if (isset($_REQUEST['tags'])) {
                $tags = uDrawUtil::sanitize_variables($_REQUEST['tags']);
            }
        ...
        function get_templates_id($templates = "") {
            if (isset($_REQUEST['templates'])) {
                $templates = uDrawUtil::sanitize_variables($_REQUEST['templates']);
            }
        ...
        function get_template_data($template_path = "") {
            ...
            if (isset($_REQUEST['template_path'])) {
                $template_path = uDrawUtil::sanitize_variables($_REQUEST['template_path']);
            }
            
            if (strlen($template_path) > 0) {
                if (is_file(UDRAW_STORAGE_DIR . $template_path)) {
                    return $this->sendResponse(base64_decode($uDrawUtil->get_web_contents(UDRAW_STORAGE_URL . $template_path)));
                }
            }
        ...
        function add_template_category($category_name = "My Cateogry", $sub_category_id ="", $type = "main") {
            ...
            if (isset($_REQUEST['category_name'])) {
                $category_name = uDrawUtil::sanitize_variables($_REQUEST['category_name']);
            }
            if (isset($_REQUEST['type'])) {
                $type = uDrawUtil::sanitize_variables($_REQUEST['type']);
            }
            if (isset($_REQUEST['sub_category_id'])) {
                $sub_category_id = uDrawUtil::sanitize_variables($_REQUEST['sub_category_id']);
            }
        ...
        function update_template_category ($category_id = 0, $parent_id = 0, $category_name = '') {
            ...
            if (isset($_REQUEST['category_id']) && isset($_REQUEST['parent_id']) && isset($_REQUEST['category_name'])) {
                $category_id = uDrawUtil::sanitize_variables($_REQUEST['category_id']);
                $category_name = uDrawUtil::sanitize_variables($_REQUEST['category_name']);
                $parent_id = uDrawUtil::sanitize_variables($_REQUEST['parent_id']);
            }
