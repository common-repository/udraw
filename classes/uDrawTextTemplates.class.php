    ...
    class uDrawTextTemplatesHandler extends uDrawAjaxBase {
        ...
        public function save () {
            ...
            if (strlen($_REQUEST['unique_id'])) {
                $name = uDrawUtil::sanitize_variables($_REQUEST['unique_id']);
            } else {
        ...
        public function save_db () {
            ...
            $uniqid = uDrawUtil::sanitize_variables($_REQUEST['unique_id']);
            $data_path = uDrawUtil::sanitize_variables($_REQUEST['data_path']);
            ...
            if (isset($_REQUEST['id']) && $_REQUEST['id'] > 0 && isset($uniqid)) {
                $id = uDrawUtil::sanitize_variables($_REQUEST['id']);
        ...
        public function load () {
            ...
            $unique_id = uDrawUtil::sanitize_variables($_REQUEST['unique_id']);
            $table = $wpdb->prefix . 'udraw_text_templates';
            $json_path = $wpdb->get_var("SELECT json FROM $table WHERE ID=$template_id AND public_key='$unique_id'");
            $json = $uDrawUtil->get_web_contents($json_path);
        ...
        function remove_text_templates_category($category = 0) {
            ...
            if (isset($_REQUEST['category'])) {
                $category = uDrawUtil::sanitize_variables($_REQUEST['category']);
            }
        ...
        function assign_text_templates ($category = 0, $publicKey = '') {
            ...
            if (current_user_can('edit_udraw_templates')) {
                if (isset($_REQUEST['public_key']) && isset($_REQUEST['category'])){
                    $publicKey = uDrawUtil::sanitize_variables($_REQUEST['public_key']);
                    $category = uDrawUtil::sanitize_variables($_REQUEST["category"]);
                }
        ...
        private function __process_fonts() {
            $uDraw = new uDraw();
            $localFontPath = uDrawUtil::sanitize_variables($_REQUEST['local_fonts_path']);
