...
if (isset($_POST['udraw_order_item_id'])) {
    ...
    $order_item_meta['udraw_product_preview'] = uDrawUtil::sanitize_variables($_POST['udraw_order_item_product_preview']);
    
    wc_update_order_item_meta(uDrawUtil::sanitize_variables($_POST['udraw_order_item_id']), 'udraw_data', $order_item_meta);
    
    // Process the change in the server to generate High-Quality PDF.
    $udraw = new uDraw();
    $udraw->generate_pdf_from_order(uDrawUtil::sanitize_variables($_GET['post']), true);
}
			...
			RacadDesigner.export_preview_image(pageIndex, function (preview_url) {
				...
				jQuery.ajax({
					...
					success: function () {
						...
						window.location.href = "?post=<?php echo uDrawUtil::esc_string($_GET['post']); ?>&action=edit&udraw_updating_design=true";
					}
				});
			});
        });
