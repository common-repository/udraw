...
if (isset($_GET['template_id'])) {
    $_template_id = uDrawUtil::sanitize_variables($_GET['template_id']);
}
...
<script type="text/javascript">
	...
	function __init_udraw(settings) {
        ...
        if (isset($_GET['template_id'])) {
            ...
            $designFile = $wpdb->get_var($wpdb->prepare("SELECT design FROM $table_name WHERE id = %d", uDrawUtil::sanitize_variables($_GET['template_id'])));
            $templateName = $wpdb->get_var($wpdb->prepare("SELECT name FROM $table_name WHERE id = %d", uDrawUtil::sanitize_variables($_GET['template_id'])));
            if (!empty($designFile)) {
                ?>
                settings.designMode = 'update';
                settings.designFile = '<?php echo uDrawUtil::esc_string($designFile); ?>';
                <?php $templateName = preg_replace('/\'/', '\\\'', $templateName); ?>
                jQuery('#udraw_template_name_txt').val('<?php echo uDrawUtil::esc_string($templateName); ?>');
                <?php
            }
        } ?>
        ...
        settings.defaultCategoryApi = 'https://draw.racadtech.com/api/category/<?php echo uDrawUtil::esc_string($_udraw_settings['udraw_designer_global_template_key']); ?>';
	    settings.defaultTemplateApi = 'https://draw.racadtech.com/api/templates/<?php echo uDrawUtil::esc_string($_udraw_settings['udraw_designer_global_template_key']); ?>';
	    <?php } ?>
    ...
    function _SaveDesign_Response(data) {
        if (data.errorMessage.length < 2) {
            ...
            var templateId = '<?php echo uDrawUtil::esc_string($_template_id); ?>';
