<?php     
    ...
    if ($_udraw_settings['udraw_customer_saved_design_page_id'] > 1) {
        if (isset($_POST['udraw_is_saving_for_later'])) {
            ...
            if( !wp_verify_nonce( $nonce, 'save_udraw_customer_design' )) {
                ...
                $access_key = (isset($_POST['udraw_save_access_key']) && strlen($_POST['udraw_save_access_key']) > 1) ? uDrawUtil::sanitize_variables($_POST['udraw_save_access_key']) : uniqid('udraw_');

                $udraw_saved_customer_design_xml = $uDrawUtil->get_web_contents(uDrawUtil::sanitize_variables(($_POST['udraw_save_product_data'])));
                $udraw_product_data_file = $access_key . '_usdf.xml';                    

                if (!$uDrawUtil->is_xml_valid($udraw_saved_customer_design_xml)) {
                    wp_die("Invalid uDraw Product Data Found.");
                }

                file_put_contents($_output_path . $udraw_product_data_file, $udraw_saved_customer_design_xml);

                $preview_url = uDrawUtil::sanitize_variables(($_POST['udraw_save_product_preview']));
                $preview_file = $access_key . '.png';
                if (startsWith($_POST['udraw_save_product_preview'], 'http')) {
                    $preview_content = $uDrawUtil->get_web_contents($path);
                ...
                $preview_url = (isset($_POST['udraw_save_product_preview']) && strlen($_POST['udraw_save_product_preview']) > 1) ? uDrawUtil::sanitize_variables($_POST['udraw_save_product_preview']) : $ifPreviewNotFound;
                ...
                if (isset($_REQUEST['udraw_price_matrix_selected_by_user']) && strlen($_REQUEST['udraw_price_matrix_selected_by_user']) > 0) {
                    $args['price_matrix_options'] = uDrawUtil::sanitize_variables($_REQUEST['udraw_price_matrix_selected_by_user']) ;
                }
                if (isset($_REQUEST['udraw_selected_variations']) && strlen($_REQUEST['udraw_selected_variations']) > 0) {
                    $args['variation_options'] = uDrawUtil::sanitize_variables($_REQUEST['udraw_selected_variations']);
                }
                if (strlen($_POST['udraw_save_access_key']) > 1) {
                    ...
                    $design_name = (isset($_POST['udraw_save_product_design_name']) && strlen($_POST['udraw_save_product_design_name']) > 1) ? uDrawUtil::sanitize_variables($_POST['udraw_save_product_design_name']) : $post->post_title;
                ...
                if (is_user_logged_in()) {
                    ...
                    foreach ($pages as $page) {
                        if ($page->ID == $_udraw_settings['udraw_customer_saved_design_page_id']) {
                            ...
                            ?>
                                <script>
                                    window.location.replace("<?php echo uDrawUtil::esc_string(get_permalink($page->ID)); ?>");
                                </script>
                            <?php                    
                            exit;
                        }
                            ...
                            <div class="modal-dialog modal-lg" role="document">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-info" data-bs-dismiss="modal" onClick="javascript: window.location.replace('<?php echo uDrawUtil::esc_string($url) ?>');"><?php _e('Continue (page will refresh)', 'udraw'); ?></button>
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    if (email_input.length > 0) {
                                        $.ajax({
                                            ...
                                            data: {
                                                ...
                                                email: email_input,
                                                design_url: '<?php echo uDrawUtil::esc_string($url) ?>',
                                                preview_url: '<?php echo uDrawUtil::esc_string($preview_url) ?>',
                                                date: '<?php echo uDrawUtil::esc_string($date) ?>',
                                                product_id: '<?php echo uDrawUtil::esc_string($post->ID) ?>'
                                            },
    ...
    if (isset($_GET['udraw_remove_template'])) {
        if (isset($_GET['udraw_access_key'])) {
            $wpdb->delete( $table_name, array( 'access_key' => uDrawUtil::sanitize_variables( ($_GET['udraw_access_key']) ) ));
