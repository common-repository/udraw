...
if (isset($_GET['udraw_access_key'])) { $udraw_access_key = uDrawUtil::sanitize_variables($_GET['udraw_access_key']); }
...
<script type="text/javascript">
    var cart_item_key = '<?php echo uDrawUtil::esc_string($_cart_item_key) ?>';
    var udraw_access_key = '<?php echo uDrawUtil::esc_string($udraw_access_key) ?>';
    var optionsFirst = '<?php echo uDrawUtil::esc_string($displayOptionsFirst) ?>';
    var convertPDF = '<?php echo uDrawUtil::esc_string($allowConvertPDF)?>';
    var bleed = <?php echo uDrawUtil::esc_string($bleed_px) ?>;
    var selectedTemplateText = "";
    if (typeof sessionID === 'undefined') {
        var sessionID = '<?php echo uDrawUtil::esc_string($user_session_id) ?>';
    }
    var productType = '<?php echo uDrawUtil::esc_string($product_type) ?>';
    ...
    jQuery(document).ready(function ($) {
        ...
        jQuery(document).on('click', '#udraw-options-page-design-btn', function () {
            if (typeof required_field_validation == 'function') {
                var validation = required_field_validation();
                if (validation) {
                    design_now_clicked();
                } else {
        ...
        $('#udraw-preview-add-to-cart-btn').click(function() {
            //Validate that total price to be not 0.
            var totalPriceBeforeAddToCart = parseFloat(jQuery('input[name="udraw_price_matrix_price"]').val());
            var totalIfNotPriceMatrix = parseFloat(jQuery('.summary.entry-summary .price .woocommerce-Price-amount').text().slice(1));
            if (totalPriceBeforeAddToCart > 0 || totalIfNotPriceMatrix > 0) {
                $('form.cart').submit();
            } else {
                alert('There was a problem adding this product to cart');
            }
        });
        ...
        $('input[name=quantity]').change(function(){
            var productPrice = '<?php echo uDrawUtil::esc_string($product->get_price()); ?>';
        ...
        jQuery('[data-udraw="uDrawBootstrap"]').on('udraw-loaded-design', function(){
            ...
            <?php if (!isset($_GET['cart_item_key'])) { ?>
                RacadDesigner.settings.user_session_id = '<?php echo uDrawUtil::esc_string($user_session_id) ?>';
            <?php } ?>
            ...
            RacadDesigner.GetPageDesignArray(true, function(xml_array){
                ...
                jQuery.ajax({
                    ...
                    data: {
                        ...
                        'folder_path' : '<?php echo uDrawUtil::esc_string($_user_session_path_url) ?>',
                        'xml_array': xml_array
                    },
        ...
        jQuery('[data-udraw="downloadPDFButton"]').on('click', function(){
            ...
            <?php if (uDraw::is_udraw_okay()) { ?>
                RacadDesigner.ExportToLayeredPDF(function(url){ 
                    ...
                    dl.setAttribute('download', '<?php echo uDrawUtil::esc_string($friendly_item_name) ?>'); 
                    dl.click(); 
                });
            <?php } else { ?>
                RacadDesigner.ExportToMultiPagePDF('<?php echo uDrawUtil::esc_string($friendly_item_name) ?>',false);
            <?php } ?>
        });
    ...
    function __init_udraw(settings) {
        ...
        settings.localImagePath = '<?php echo uDrawUtil::esc_string($_local_image_path) ?>';    
        ...
        settings.language = '<?php echo uDrawUtil::esc_string($_udraw_settings['udraw_designer_language']); ?>';
        ...
        settings.displayOrientation = '<?php echo uDrawUtil::esc_string($_udraw_settings['udraw_designer_display_orientation']); ?>';
        settings.excludeBleed = false;
        settings.designer_ui = '<?php echo uDrawUtil::esc_string($designer_ui) ?>';
        ...
        <?php if ($_udraw_settings['designer_check_dpi']) { ?>
            ...
            settings.minimum_dpi_requirement = parseFloat('<?php echo uDrawUtil::esc_string($_udraw_settings['designer_minimum_dpi']) ?>');
                ...
                if (!isset($isBlankCanvas)) {
                    ...
                    if (count($templateId) > 0) {
                        ...
                        if (count($templateId) > 1) {
                            ...
                            echo "settings.designFile = '". uDrawUtil::esc_string($designURL) ."';";
                        }
                        echo "settings.privateTemplateApi = '". admin_url( 'admin-ajax.php' ) ."?action=udraw_related_templates&template_id=" . uDrawUtil::esc_string($templateId[0]) . "';";
                    }
        ...
        settings.FBappID = '<?php echo uDrawUtil::esc_string($_udraw_settings['designer_facebook_app_id']) ?>';
        <?php } else { ?>
        settings.FBappID = '';
        <?php } ?>
        <?php if ($_udraw_settings['designer_enable_instagram_functions']) { ?>
        settings.instagramClientID = '<?php echo uDrawUtil::esc_string($_udraw_settings['designer_instagram_client_id'])?>';
        <?php } else { ?>
    ...
    function __get_app_id(){
        RacadDesigner.settings.FBappID = '<?php echo uDrawUtil::esc_string($_udraw_settings['designer_facebook_app_id'])?>';
    }
	
	function __loaded_udraw() {
	    <?php
            if (isset($session_id)) {
                echo 'RacadDesigner.session_id="' . uDrawUtil::esc_string($session_id) . '";';
            }
            ...
            if( strlen($_cart_item_key) > 0 ) {
                ...
                echo "jQuery('input[name=\"udraw_product_cart_item_key\"]').val('". uDrawUtil::esc_string($_cart_item_key) ."');";
                
                if($cart_item) {
                    if( isset($cart_item['udraw_data']) ) {
                        // Insert previous data
                        echo "jQuery('input[name=\"udraw_product_data\"]').val('" . uDrawUtil::esc_string(UDRAW_STORAGE_URL) . uDrawUtil::esc_string($cart_item['udraw_data']['udraw_product_data']) ."');";
                        echo "jQuery('input[name=\"udraw_product_preview\"]').val('". uDrawUtil::esc_string($cart_item['udraw_data']['udraw_product_preview']) ."');";
                        ...
                        if (isset($cart_item['udraw_data']['udraw_product_data'])) {
                            if (strlen($cart_item['udraw_data']['udraw_product_data']) > 0) {
                                if (strlen($cart_item['udraw_data']['udraw_product_data']) < 100) {
                                    ...
                                    echo "jQuery.ajax({ method: 'GET', dataType: 'text', url: '". uDrawUtil::esc_string(UDRAW_STORAGE_URL) . uDrawUtil::esc_string($cart_item['udraw_data']['udraw_product_data']) ."', success: function(response) { RacadDesigner.Legacy.loadCanvasDesignXML(Base64.decode(response)); },error: function(error) {console.error(error);} });";
                                } else {
                                    // older db format
                                    echo "RacadDesigner.Legacy.loadCanvasDesignXML(Base64.decode('". uDrawUtil::esc_string($cart_item['udraw_data']['udraw_product_data']) ."'));";
                                }
                            }
                                
                        } else if (isset($cart_item['udraw_data']['udraw_price_matrix_design_data'])) {
                            echo "RacadDesigner.Legacy.loadCanvasDesignXML(Base64.decode('". uDrawUtil::esc_string($cart_item['udraw_data']['udraw_price_matrix_design_data']) ."'));";
                        }
            ...
            if( isset($_GET['udraw_access_key']) ) {
                ...
                if (strlen($design['design_data']) > 1 ) {
                    if (strlen($design['design_data']) < 100) {
                        ...
                        echo "jQuery.ajax({ method: 'GET', dataType: 'text', url: '". uDrawUtil::esc_string($design_file_path) ."', success: function(response) { RacadDesigner.Legacy.loadCanvasDesignXML(Base64.decode(response)); },error: function(error) {console.error(error);} });";                        
                    } else {
                        // older db format
                        echo "RacadDesigner.Legacy.loadCanvasDesignXML(Base64.decode('". uDrawUtil::esc_string($design['design_data']) ."'));";
                    }
                }
                ...
                if ($design['variation_options'] !== NULL) {
                    ...
                    for ($i = 0; $i < count($variation_options); $i++) {
                        ...
                        ?>
                            jQuery('[name="<?php echo uDrawUtil::esc_string($name) ?>"]').val('<?php echo uDrawUtil::esc_string($value) ?>');
                        <?php
                    }
                }
            }
        ...
        jQuery.ajax({
            ...
            success: function (files_array) {
                for (var i = 0; i < files_array.length; i++) {
                    files_array[i] = files_array[i].replace('<?php echo uDrawUtil::esc_string($_user_session_path_url) ?>', '');
                }
    ...
    function create_cart_xml_file (numOfPages, callback) {
        ...
        jQuery.ajax({
            ...
            data: {
                ...
                'folder_path' : '<?php echo uDrawUtil::esc_string($_user_session_path_url) ?>',
                'cart_folder_path' : '<?php echo uDrawUtil::esc_string($_cart_path_url) ?>',
