...
<div id="pdf-block-product-ui" style="display: none;">
                        ...
                        <div>
                            ...
                            <?php 
                                foreach ($blocks_array as $block) {                                
                                    ...
                                    if (isset($block_json)) {
                                        $thumbnailSmall = isset($block_json['ThumbnailSmall']) ? uDrawUtil::esc_string($block_json['ThumbnailSmall']) : '';
                                        $thumbnailLarge = isset($block_json['ThumbnailLarge']) ? uDrawUtil::esc_string($block_json['ThumbnailLarge']) : '';
                                        $productName = isset($block_json['ProductName']) ? uDrawUtil::esc_string($block_json['ProductName']) : '';
                                        echo '<option value="'. uDrawUtil::esc_string($block) . '" data-thumb-small="'. $thumbnailSmall .'" data-thumb-large="'. $thumbnailLarge .'">' . $productName . '</option>';
                                    }
                                }
                            ?>
                            </select>
                    ...
                    <div class="col-xs-12 col-md-12 col-lg-12" style="float: none;">
                        <div id="w2p-pdf-template-wrapper">
                            ...
                            <?php if ($_udraw_settings['approve_proof_text'] == "") { ?>
                                <div class="accept-proof"><p class="acceptProof"><span class="approval">ARTWORK APPROVAL: </span>By clicking the Approve &amp; Continue button, I am verifying that I have customized this template to my satisfaction and give <?php echo uDrawUtil::esc_string(get_bloginfo( 'name' )); ?> authorization to print the artwork exactly as presented here. I understand that <?php echo uDrawUtil::esc_string(get_bloginfo( 'name' )); ?> will not be held responsible for any typos or errors that I have inputted or any logo reproduction quality issues based on my provided art file.</p></div>
                            <?php } else { ?>
                                <div class="accept-proof"><p class="acceptProof"><span class="approval">ARTWORK APPROVAL: </span><?php echo stripcslashes(htmlspecialchars_decode(esc_html($_udraw_settings['approve_proof_text']))); ?></p></div>
                            <?php } ?>
    ...
    <?php echo stripcslashes(htmlspecialchars_decode(esc_html($_udraw_settings['udraw_pdf_template_html_hook'])));?>
</div>
...
<script type="text/javascript">
    ...
    var currentBlockId = '<?php echo uDrawUtil::esc_string($blocks_array[0]); ?>';
    <?php
    ...
    if( isset($_GET['cart_item_key']) ) {
        ...
        $cart_item = uDrawUtil::sanitize_variables($cart[$_GET['cart_item_key']]);
        if($cart_item) {
            if( isset($cart_item['udraw_data']['udraw_pdf_block_product_data']) ) { ?>
                var dataJSON = '<?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($cart_item["udraw_data"]["udraw_pdf_block_product_data"]))); ?>';
                _previous_pdf_block_entries = JSON.parse(dataJSON);
                currentBlockId = '<?php echo uDrawUtil::esc_string($cart_item['udraw_data']['udraw_pdf_block_product_id']); ?>';
            <?php }
        }
    }
        ...
        if (strlen($design['design_data']) > 1 ) {
            if (strlen($design['design_data']) < 100) {
                if (is_file(UDRAW_STORAGE_DIR . $design['design_data'])) {
                    ...
                    var dataJSON = '<?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($_saved_block_data))); ?>';
                    _previous_pdf_block_entries = JSON.parse(dataJSON);
                <?php }
            } else { ?>
                var dataJSON = '<?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($design['design_data']))); ?>';
                _previous_pdf_block_entries = JSON.parse(dataJSON);
            <?php }   
        }
    }
    ...
    <?php if ($_udraw_settings['goepower_hide_labels_on_text_input']) { echo uDrawUtil::esc_string("hideTextLabels = true;"); } ?>
    ...
    jQuery(document).ready(function ($) {
        ...
        jQuery(document).on('udraw_price_matrix_load_templates', function(event) {
            if (typeof event.template_id !== 'undefined' && event.template_id !== null) {
                ...
                var template_id = '<?php echo isset($block_product['UniqueID']) ? uDrawUtil::esc_string($block_product['UniqueID']) : null ?>';
        ...
        <?php if ($_udraw_settings['goepower_pdf_preview_auto_update']) { ?>
            ...
            $(_pdf_container_element).on('change', '.w2p-pdf-template-input-checkbox', function () { w2p_bm.process_preview(); });
            $(_pdf_container_element).on('change', '.w2p-pdf-template-input-color', function () { w2p_bm.process_preview(); });
            $(_pdf_container_element).on('change', '.block-image-modal-selected', function () { w2p_bm.process_preview(); });
        <?php } ?>

        <?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($_udraw_settings['udraw_pdf_template_js_hook']))); ?>
   });
</script>
<style>
    ...
    #pdf-block-product-ui #udraw-bootstrap .tox .tox-tbtn--bespoke .tox-tbtn__select-label { 
        width: auto !important; 
    }

    #pdf-block-product-ui #udraw-bootstrap .tox-fullscreen {
        z-index: 999999;
    }
    ...
    <?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($_udraw_settings['udraw_pdf_template_css_hook']))); ?>
