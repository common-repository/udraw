...
$udraw_access_key = (isset($_GET['udraw_access_key'])) ? uDrawUtil::sanitize_variables($_GET['udraw_access_key']) : NULL;
            ...
            <div class="col-lg-12 col-md-12 col-sm-12">
                    ...
                    <div class="col-lg-12">
                        ...
                        <?php 
                            foreach ($xmpie_array as $xmpie) {                                
                                ...
                                echo '<option value="'. uDrawUtil::esc_string($xmpie_json['UniqueID']) . '">' . uDrawUtil::esc_string($xmpie_json['ProductName']) . '</option>';
                            }
                        ?>
                        </select>
                    </div>                 
                ...
                <div class="col-lg-8">
                    <div class="product_image_preview" style="text-align: center;">
                        ...
                        ?>
                        <img src="<?php echo uDrawUtil::esc_string($product_thumbnail); ?>">
                    </div>
                    <div id="pdf-xmpie-thumbnail-preview" class="touchcarousel three-d" style="border: 1px solid rgb(203, 203, 203); float:right;">
                        <!--<img src="<?php echo uDrawUtil::esc_string($uDrawPdfXMPie->get_product($xmpie_array[0])['ThumbnailLarge']); ?>" />-->
                    </div>
                    <div id="loadingDiv" style="float:right; display:none;" >
                            <img src="<?php echo uDrawUtil::esc_string(UDRAW_PLUGIN_URL) ?>assets/includes/loading-animation.gif" />
                    </div>
...
if( isset($_GET['udraw_access_key']) ) {
    ...
    if (strlen($design['design_data']) > 1 ) {
        ...
        $contents = $uDrawUtil->get_web_contents(UDRAW_STORAGE_URL . '/' . stripslashes($design['design_data']));
        echo '<script> _previous_pdf_xmpie_entries = jQuery.parseJSON(\''. uDrawUtil::esc_string($contents) .'\');</script>';
    }
}
?>

<script type="text/javascript">
    var use_colour_palette = <?php echo uDrawUtil::esc_string($use_colour_palette) ?>;
    ...
    var currentXMPieId = '<?php echo uDrawUtil::esc_string($xmpie_array[0]); ?>';
    
    <?php if($isMultiXMPie){ ?>
    var currentXMPieUniqueID = jQuery('#_product_selection').val();
    <?php } else { ?>
    var currentXMPieUniqueID = '<?php echo uDrawUtil::esc_string($uDrawPdfXMPie->get_product($xmpie_array[0])['UniqueID']); ?>';
    <?php } ?>
    var currentPDFDoc = '';
    ...
    function __process_xmpie_preview_success(result) {
        ...
        <?php } else { ?>
        jQuery("#previewDiv").html("<iframe src='<?php echo uDrawUtil::esc_string(UDRAW_PLUGIN_URL) ?>/assets/pdfjs/web/viewer.php?ps=no&file=" + result.src + "&ps=<?php echo uDrawUtil::esc_string($allowPDFPrintSave); ?>' style='width:100%; min-height: 475px;' allowfullscreen webkitallowfullscreen></iframe>");
        <?php } ?>
    ...
    function __update_cart_form(callback) {
        Xmpie.getEntries(function(xmpieJsonEntry){
            jQuery('<input>').attr({
                ...
                value: '<?php echo uDrawUtil::esc_string($xmpie_array[0]); ?>'
            }).appendTo('form.cart');
