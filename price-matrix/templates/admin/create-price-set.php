    ...
    <table style="width:100%; padding-bottom:10px;">
        <tr>
            <td>                
                ...
                <input type="text" name="price-matrix-name-textbox" id="price-matrix-name-textbox" value="<?php echo uDrawUtil::esc_string($_name); ?>" style="width:70%;font-size:12pt;background-color: rgb(234, 232, 255);">
            </td>
...
<script type="text/javascript">         
    ...
    function display_udraw_price_matrix_preview(access_key) {
        eFileName = '<?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js(admin_url('admin-ajax.php') . '?action=udraw_price_matrix_get&price_matrix_id='. $access_key))); ?>';
...
<?php
    function price_matrix_tax_meta() {
        $cats = array();
        $access_key = (isset($_REQUEST['access_key'])) ? uDrawUtil::sanitize_variables($_REQUEST['access_key']) : 0;
        $uDrawPriceMatrix = new uDrawPriceMatrix();
    ...
    function print_tax_inputs( $term, $taxonomy_name, $cats, $level ) { ?>
        <li>
            <input type="checkbox" id="_udraw_price_matrix_cat_<?php echo uDrawUtil::esc_string($term->term_id) ?>" name="_udraw_price_matrix_cat_<?php echo uDrawUtil::esc_string($term->term_id) ?>" <?php if ( is_array( $cats ) and in_array( $term->term_id, $cats )) echo 'checked="checked"' ?> /><?php echo uDrawUtil::esc_string($term->name); ?>
        </li>
    <?php 
    ...
    function udraw_price_matrix_layout_html($_content, $access_key) {
        ...
        <script>
            ...
            var _price_matrix = {
                _access_key: '<?php echo uDrawUtil::esc_string($access_key); ?>',
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" class="button button-default" data-bs-dismiss="modal"><?php _e('Cancel','udraw'); ?></button>
    ...
    function udraw_price_matrix_source_html ($_content, $_action) {
        ...
        <textarea class="wp-editor-area hidden" rows="20" cols="40" name="section_content2" id="section_content2" style="display: none !important;"><?php echo uDrawUtil::esc_string($_content);?></textarea>
        ...
        <input type="hidden" name="action" value="<?php echo uDrawUtil::esc_string($_action); ?>" />
        <?php if ($_action == "update") { ?>
            <input type="hidden" name="access_key" id="access_key" value="<?php echo uDrawUtil::esc_string($_REQUEST['access_key']); ?>" />
        <?php }
    }
    ...
    function udraw_price_matrix_preview_html($uDraw) {
        ...
        if (isset($_REQUEST['access_key'])) {            
            $price_matrix = $uDrawPriceMatrix->get_price_matrix_by_key(uDrawUtil::sanitize_variables($_REQUEST['access_key']));
        ...
        <div class="divContainer">
            ...
            <div>
                ...
                <div style="display:none;">
                    ...
                    <input type="text" value="<?php echo uDrawUtil::esc_string($_font_color); ?>" id="price-matrix-font-color" />
                </div>
                <div style="display:none;">
                    <label for="price-matrix-background-color" style="font-size:12pt; vertical-align:top;">Background:&nbsp;</label>
                    <input type="text" value="<?php echo uDrawUtil::esc_string($_background_color); ?>" id="price-matrix-background-color" />
                </div>
