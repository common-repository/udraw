...
<style type="text/css">
    ...
    #canvas select, #txtQty {
        ...
        -moz-appearance: none;     
        background: <?php echo uDrawUtil::esc_string($_background_color); ?> url(<?php echo uDrawUtil::esc_string(UDRAW_PLUGIN_URL) ?>assets/includes/arrowdown.png) no-repeat 98% center; /* #0088cc url(img/select-arrow.png) no-repeat 90% center; */
        text-indent: 0.01px;
        text-overflow: "";
        color: <?php echo uDrawUtil::esc_string($_font_color); ?> !important;
        padding: 5px;
...
<form id="price_matrix_form" method="post" action="">
    ...
    <?php if (isset($_GET['cart_item_key'])) { ?>
        <input type="hidden" value="<?php echo uDrawUtil::esc_string($_GET['cart_item_key']); ?>" name="cart_item_key" />
    <?php } ?>
...
<script type="text/javascript"> 
    ...
    var design_data = '';
    var design_preview = '';
    var measurement_unit_label = '<?php echo uDrawUtil::esc_string($_measurement_unit) ?>';
    var priceMatrixObj;
    ...
    jQuery(document).ready(function($) {
        ...
        $('#fileuploadA').fileupload({
            ...
            add: function (e, data) {
				...
				var uploadErrors = [];
                var getAccepted = jQuery('#fileuploadA').attr('accept');
				var accepted = getAccepted.replace(/\./g, '').replace(/,/g, '|');
				var acceptFileTypes = new RegExp("\/(" + accepted + ")$" ,"i");
				
				if(data.originalFiles[0]['type'].length && !acceptFileTypes.test(data.originalFiles[0]['type'])) {
					uploadErrors.push('\nNot an accepted file type.');
                    uploadErrors.push('\nAccepted File Types: ' + getAccepted)
				}

                if(data.originalFiles.length > maxlimit) {
                    uploadErrors.push('\nYou have reached the maximum upload limit.');
                    uploadErrors.push('\nMaximum number of uploads allowed are ' + maxlimit)
                }
        ...
        $('#fileuploadB').fileupload({
            ...
            add: function (e, data) {
                var maxlimit = '<?php echo get_post_meta($post->ID, '_max_files_allowed', true);?>';
				var uploadErrors = [];
                var getAccepted = jQuery('#fileuploadB').attr('accept');
				var accepted = getAccepted.replace(/\./g, '').replace(/,/g, '|');
				var acceptFileTypes = new RegExp("\/(" + accepted + ")$" ,"i");
        ...
        $('#add-to-cart-btn-span').click(function() {
            var _upload_path = '<?php echo uDrawUtil::esc_string($_udraw_settings['udraw_price_matrix_upload_path']) . $_session_upload_id . "/"; ?>';
                ...
                if (isset($_GET['cart_item_key'])) {
                    foreach ($woocommerce->cart->get_cart() as $cart_item_key => $values) {
                        if ($cart_item_key == $_GET['cart_item_key']) {
                            ?>
                                var selectedOutput = '<?php echo uDrawUtil::esc_string($values['udraw_data']['udraw_price_matrix_selected_options']); ?>';
                                var selectedPMOptions = '<?php echo uDrawUtil::esc_string($values['udraw_data']['udraw_price_matrix_selected_options_object']); ?>';
                                var projected_pricing = '<?php echo uDrawUtil::esc_string($values['udraw_data']['udraw_price_matrix_projected_pricing']); ?>';
                                var _idx = '<?php echo uDrawUtil::esc_string($values['udraw_data']['udraw_price_matrix_selected_options_idx']); ?>';
                                selectedSaved = _idx.split(',');
                                selectedDefault = _idx.split(',');
                                selectedByUser = _idx.split(',');
                                loadedFromCart = true;
                                setTimeout(function() {
                                    jQuery("#txtQty option[value='<?php echo uDrawUtil::esc_string($values['udraw_data']['udraw_price_matrix_qty']); ?>']").prop('selected', "selected");
