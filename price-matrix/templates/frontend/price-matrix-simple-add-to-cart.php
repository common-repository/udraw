...
if ($disableCartPreviewThumbnail == "yes" ) { $is_disable_cart_preview_thumbnail = true; } 
...
<form class="cart price_matrix_form" method="post" enctype='multipart/form-data'>
    ...
    <input type="hidden" value="<?php echo uDrawUtil::esc_string($price_matrix_object[0]->name); ?>" name="udraw_price_matrix_name" />
                    ...
                    <div class="divContainer">
                        ...
                        <div class="Total" style="width:100%; text-align: right; padding-bottom: 10px !important;">
                            ...
                            <span style="font-size: 18pt;color: rgb(0, 128, 0);font-weight: bold;">
                                <img src="<?php echo uDrawUtil::esc_string(UDRAW_PLUGIN_URL) ?>assets/includes/price-matrix-loading.gif" id="udraw-price-matrix-loading-img" />
    ...
    <table style="width: 100%;">
        <tr>
            <td>
			<?php
			if ($settings['show_customer_preview_before_adding_to_cart']) {
                if(($is_design_product || $is_upload_product) && $bypass_design !== 'yes') { ?>
                    <button type="submit" <?php echo htmlspecialchars($style, ENT_NOQUOTES); ?> id="udraw-options-submit-form-btn" class="single_add_to_cart_button btn btn-primary">Preview & Add to Cart</button>
                <?php } else { ?>
                    <button type="submit" <?php echo htmlspecialchars($style, ENT_NOQUOTES); ?> id="udraw-options-submit-form-btn" class="single_add_to_cart_button btn btn-primary"><?php echo esc_html($product->single_add_to_cart_text() ); ?></button>
                <?php }
            } else { ?>
                <button type="submit" <?php echo htmlspecialchars($style, ENT_NOQUOTES); ?> id="udraw-options-submit-form-btn" class="single_add_to_cart_button btn btn-primary"><?php echo esc_html($product->single_add_to_cart_text() ); ?></button>
            <?php } ?>
    ...
    <tr class="udraw_action_row design_row">
        ...
        <?php if ($is_upload_product && !$is_design_product_update && !$is_double_upload_product) { ?>
            <td>
                ...
                <input style="display: none; visibility: hidden; width: 0; height: 0;" id="fileuploadA" type="file" name="files[]" accept="<?php echo uDrawUtil::esc_string($valid_extensions) ?>" multiple>
            </td>
        <?php } else if ($is_upload_product && !$is_design_product_update && $is_double_upload_product) { ?>
            ...
            <td>
                ...
                <input style="display: none; visibility: hidden; width: 0; height: 0;" id="fileuploadB" type="file" name="files[]" accept="<?php echo uDrawUtil::esc_string($valid_extensions) ?>" multiple>
            </td>
        <?php } else { ?>
            <td></td>
        <?php } ?>
...
<script>
    ...
    function display_udraw_price_matrix_preview() {
        eFileName = '<?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js(admin_url('admin-ajax.php') . '?action=udraw_price_matrix_get&price_matrix_id='. $udraw_price_matrix_access_key))); ?>';
        priceMatrixObj = PriceMatrix({
            url: eFileName,
            key: '<?php echo uDrawUtil::esc_string(uDraw::get_udraw_activation_key()); ?>',
    ...
    function required_field_validation() {
        var _fields_validation = true;
        jQuery('#udraw-price-matrix-ui-container input:required, #udraw-price-matrix-ui-container select:required, #udraw-price-matrix-ui-container textarea:required').each(function() {
            if (typeof jQuery(this) !== 'undefined') {
                if (jQuery(this).attr('data-control-type') == 'pm_dropdown') {
                    if (typeof jQuery(this).attr('data-placeholder') !== 'undefined') {
                        var selectVal = jQuery(this).find('option:selected').text().toLowerCase();
                        var placeValue = jQuery(this).attr('data-placeholder');
    ...
    jQuery(document).ready(function ($) {
        ...
        <?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($settings['udraw_price_matrix_js_hook']))); ?>

        //To restrict submitting price matrix form when "Enter" key is pressed.
        jQuery('form.cart').on('keyup keypress', function(event) {
        ...
        jQuery(document).on('udraw_price_matrix_display_price', function(event){
            ...
            if (parseFloat(event.price_object.Price) > 0) {
            ...
            jQuery(document).trigger({
                type: 'udraw_after_addTocart_disable_on_pricezero',
                price_object: parseFloat(event.price_object.Price)
			});
        });

        var price_matrix_placement = '<?php echo uDrawUtil::esc_string($settings['udraw_price_matrix_placement']) ?>';
...
<style>
    ...
    <?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($settings['udraw_price_matrix_css_hook']))); ?>
</style>
