...
if (!class_exists('uDrawPriceMatrix')) {
    class uDrawPriceMatrix {
        ...
        public function process_header_request() {
            ...
            if (isset($_POST["udraw_price_matrix_submit"])) {
                if ($_POST["udraw_price_matrix_submit"] == "true") {
                    ...
                    $price_matrix_data['udraw_data']['udraw_price_matrix_selected_options_idx'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_selected_options_idx']);
                    $price_matrix_data['udraw_data']['udraw_price_matrix_selected_options'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_selected_options']);
                    $price_matrix_data['udraw_data']['udraw_price_matrix_selected_options_object'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_selected_options_object']);
                    $price_matrix_data['udraw_data']['udraw_price_matrix_projected_pricing'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_projected_pricing']);
                    if (!is_null($_POST['udraw_price_matrix_width'])) {
                        $price_matrix_data['udraw_data']['udraw_price_matrix_width'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_width']);
                        $price_matrix_data['udraw_data']['udraw_price_matrix_height'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_height']);
                    }
                    $price_matrix_data['udraw_data']['udraw_price_matrix_price'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_price']);
                    $price_matrix_data['udraw_data']['udraw_price_matrix_qty'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_qty']);
                    $price_matrix_data['udraw_data']['udraw_price_matrix_records'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_records']);
                    if (isset($_POST['udraw_price_matrix_uploaded_files'])) {
                        $price_matrix_data['udraw_data']['udraw_price_matrix_uploaded_files'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_uploaded_files']);
                    }
                    if (isset($_POST['udraw_price_matrix_design_data'])) {
                        $price_matrix_data['udraw_data']['udraw_price_matrix_design_data'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_design_data']);
                    }
                    if (isset($_POST['udraw_price_matrix_design_preview'])) {
                        $price_matrix_data['udraw_data']['udraw_price_matrix_design_preview'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_design_preview']);
                    }
                    if (isset($_POST['udraw_custom_design_name'])) {
                        $price_matrix_data['udraw_data']['udraw_custom_design_name'] = uDrawUtil::sanitize_variables($_POST['udraw_custom_design_name']);
                    }
                    $price_matrix_data['udraw_data']['udraw_price_matrix_url'] = get_permalink( $post->ID );            
                    $price_matrix_data['udraw_data']['udraw_price_matrix_name'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_name']);
        ...
        public function woocommerce_cart_shipping_packages($packages) {
			if (isset($packages)) {
                foreach( $packages as $packageKey => $package ) {
					foreach( $package['contents'] as $product_details ) {
						if (isset($product_details['udraw_data'])) {
							$udraw_data = $product_details['udraw_data'];
							$package_data = $product_details['data'];
							if (isset($udraw_data["udraw_price_matrix_shipping_dimensions"])) {
								if (strlen($udraw_data["udraw_price_matrix_shipping_dimensions"]) > 0) {
									...
									if (count($parsed) == 1) {
										$package_data->set_weight( $parsed[0]->weight );
										$package_data->set_length( $parsed[0]->length );
										$package_data->set_width( $parsed[0]->width );
										$package_data->set_height( $parsed[0]->height );
									} else if (count($parsed) > 1) {
										// Set Qty to trigger multiple parcels.
										$product_details['quantity'] = $product_details['quantity'] + (count($parsed) - 1);
									}                                
								}
							} else {
								if (isset($udraw_data["udraw_price_matrix_weight"])) {
									if (strlen($udraw_data["udraw_price_matrix_weight"]) > 0) {
										$package_data->weight = $udraw_data["udraw_price_matrix_weight"];
										$package_data->length = $udraw_data["udraw_price_matrix_length"];
									}
								}
								if (isset($udraw_data["udraw_price_matrix_width"])) {
									if (strlen($udraw_data["udraw_price_matrix_width"]) > 0) {
										$package_data->width = $udraw_data["udraw_price_matrix_width"];
										$package_data->height = $udraw_data["udraw_price_matrix_height"];
									}
								}		
							}
						}
        ...
        public function uDraw_price_matrix_manage_page() {
            ...
            if (file_exists(plugin_dir_path(__FILE__) . 'includes/raw js/price-matrix-ui.js')) {
                wp_register_script('price-matrix-ui-js', plugins_url('includes/raw js/price-matrix-ui.js', __FILE__));
            } else {
        ...
        public function register_price_matrix_frontend() {
            ...
            echo '<script>var license_key = "' . uDrawUtil::esc_string($license_key) .'";</script>';
            // Register Scripts
            wp_register_script('udraw_base64_js', 'https://udraw-app.racadtech.com/includes/thirdparty/misc/webtoolkit.base64.js');
            if (file_exists(plugin_dir_path(__FILE__) . 'includes/raw js/price-matrix.js')) {
                wp_register_script('price-matrix-js', plugins_url('includes/raw js/price-matrix.js', __FILE__));
            ...
            if (strlen($output) === 0) {
                ...
                if (isset($that->meta['udraw_data'])) {
                    ...
                    if (isset($udraw_data['udraw_price_matrix_selected_options']) && isset($udraw_data['udraw_price_matrix_qty']) ) {                                
                        ...
                        $extra_meta_list[] = wp_kses_post('Quantity: ' . $udraw_data['udraw_price_matrix_qty'] );
                        ...
                        if (isset($udraw_data['udraw_price_matrix_selected_options_object'])) {
                            ...
                            for ($x = 0; $x < count($selected_options); $x++) {
                                ...
                                $extra_meta_list[] = wp_kses_post($selected_options[$x]->name . ': '.  $selected_options[$x]->value);
                            }
                        } else {
                            ...
                            foreach ($selected_options as $option => $value) {
                                ...
                                $extra_meta_list[] = wp_kses_post( $option . ': ' . $value[0] );
                            }
                        }

                        $extra_meta_list = apply_filters('udraw_pricematrix_order_items_meta', $extra_meta_list);

                        $output .= implode( "<br />", $extra_meta_list );
                    }
                }
            }
        ...
        public function add_cart_item_data($cart_item_meta, $product_id) {
            ...
            if ($updateCartMeta) {
                $cart_item_meta['udraw_data']['udraw_price_matrix_name'] = (isset($_POST['udraw_price_matrix_name'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_name']) : NULL;
                $cart_item_meta['udraw_data']['udraw_custom_design_name'] = (isset($_POST['udraw_custom_design_name'])) ? uDrawUtil::sanitize_variables($_POST['udraw_custom_design_name']) : NULL;
                $cart_item_meta['udraw_data']['udraw_price_matrix_selected_options_idx'] = (isset($_POST['udraw_price_matrix_selected_options_idx'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_selected_options_idx']) : NULL;
                $cart_item_meta['udraw_data']['udraw_price_matrix_selected_options'] = (isset($_POST['udraw_price_matrix_selected_options'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_selected_options']) : NULL;
                $cart_item_meta['udraw_data']['udraw_price_matrix_selected_options_object'] = (isset($_POST['udraw_price_matrix_selected_options_object'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_selected_options_object']) : NULL;
                $cart_item_meta['udraw_data']['udraw_price_matrix_projected_pricing'] = (isset($_POST['udraw_price_matrix_projected_pricing'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_projected_pricing']) : NULL;
                $cart_item_meta['udraw_data']['udraw_price_matrix_price'] = (isset($_POST['udraw_price_matrix_price'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_price']) : NULL;
                $cart_item_meta['udraw_data']['udraw_price_matrix_qty'] = (isset($_POST['udraw_price_matrix_qty'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_qty']) : NULL;
                $cart_item_meta['udraw_data']['udraw_price_matrix_records'] = (isset($_POST['udraw_price_matrix_records'])) ? uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_records']) : NULL;

                if (isset($_POST['udraw_price_matrix_weight'])) {
                    if (!is_null($_POST['udraw_price_matrix_weight'])) {
                        $cart_item_meta['udraw_data']['udraw_price_matrix_weight'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_weight']);
                        $cart_item_meta['udraw_data']['udraw_price_matrix_length'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_length']);
                    }
                }
                if (isset($_POST['udraw_price_matrix_width'])) {
                    if (!is_null($_POST['udraw_price_matrix_width'])) {
                        $cart_item_meta['udraw_data']['udraw_price_matrix_width'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_width']);
                        $cart_item_meta['udraw_data']['udraw_price_matrix_height'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_height']);
                    }
                }
                if (isset($_POST['udraw_price_matrix_shipping_dimensions'])) {
                    if (!is_null($_POST['udraw_price_matrix_shipping_dimensions'])) {
                        $cart_item_meta['udraw_data']['udraw_price_matrix_shipping_dimensions'] = uDrawUtil::sanitize_variables($_POST['udraw_price_matrix_shipping_dimensions']);
                    }
                }
            }
            ...
            if (isset($item['udraw_data'])) {
                ...
                if( isset($item['udraw_data']) ) {
                    if (isset($item['udraw_data']['udraw_price_matrix_selected_options_object'])&& strlen($item['udraw_data']['udraw_price_matrix_selected_options_object']) > 0) {
                        ...
                        ?>
                        <a class='button button-small button-secondary udraw-price-matrix thickbox' href="#TB_inline?width=600&height=550&inlineId=<?php echo uDrawUtil::esc_string($unique_price_matrix_id); ?>" onclick="javascript:window.tb_show('Price Matrix Options', '#TB_inline?width=600&height=550&inlineId=<?php echo uDrawUtil::esc_string($unique_price_matrix_id); ?>');" style='width: 150px; text-align: center;'>Price Matrix Options</a>
                        <div id="<?php echo uDrawUtil::esc_string($unique_price_matrix_id); ?>" style="display:none;">
                                ...
                                <table style="width: 100%; height: auto; border: 1px solid black;">
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td style="border: 1px solid grey;"><?php echo uDrawUtil::esc_string($item['udraw_data']['udraw_price_matrix_qty']) ?></td>
                                        </tr>
                                        <?php
											...
											for ($x = 0; $x < count($options); $x++) {
												?>
                                                <tr>
                                                    <td style="border: 1px solid grey;"><?php echo uDrawUtil::esc_string($options[$x]->name); ?></td>
                                                    <td style="border: 1px solid grey;"><?php echo uDrawUtil::esc_string($options[$x]->value); ?></td>
                                                </tr>
                                                <?php
											}
                                        ?>
        ...
        public function checkout_update_order_meta($order_id) {
            ...
            for ($x = 0; $x < count($item_keys); $x++) {
                ...
                if (strlen($udraw_data['udraw_price_matrix_qty']) && !isset($udraw_data['udraw-book-color-pages']) && !isset($udraw_data['udraw-book-bw-pages'] )) { 
                ...
                if (isset($udraw_data['udraw_price_matrix_selected_options_object'])) {
                    ...
                    if (strlen($udraw_data['udraw_price_matrix_selected_options'])) {
                        ...
                        $selected_options = apply_filters('udraw_pricematrix_checkout_update_order_meta', $selected_options);
                        foreach ($selected_options as $option => $value) {
                            $sub_val_str = '';
                            foreach($value as $sub_val) {
                                $sub_val_str .= $sub_val . '</br>';
        ...
        public function get_product_price_matrix_key($product_id) {
            ...
            if ($is_price_matrix_set_on_product == "yes") {
                ...
                if (!empty($selected_price_matrix_on_product)) {
            ...
            if (isset($_REQUEST['name']) && isset($_REQUEST['xml'])) {
                ...
                if (isset($_REQUEST['access_key']) && strlen($_REQUEST['access_key']) > 0) {
                    $access_key = $this->update_price_matrix(
                        uDrawUtil::sanitize_variables($_POST['name']), 
                        esc_xml($_POST['xml']),
                        uDrawUtil::sanitize_variables($_POST['access_key'])
                    );
                } else {
                    $access_key = $this->create_price_matrix(
                        uDrawUtil::sanitize_variables($_REQUEST['name']), 
                        esc_xml($_REQUEST['xml']), 
                ...
                isset($_REQUEST['disable_file_upload']) && isset($_REQUEST['disable_design_online']) && isset($_REQUEST['measurement_label']) ) {
                    if (isset($_REQUEST['udraw_template_id'])) {
                        $linked_template = uDrawUtil::sanitize_variables($_REQUEST['udraw_template_id']);
                    } else {
                        $linked_template = NULL;
                    }

                    $this->handle_ajax_update_preview_settings($access_key, uDrawUtil::sanitize_variables($_REQUEST['font_color']), uDrawUtil::sanitize_variables($_REQUEST['background_color']), uDrawUtil::sanitize_variables($_REQUEST['disable_file_upload']), uDrawUtil::sanitize_variables($_REQUEST['disable_design_online']), $linked_template, uDrawUtil::sanitize_variables($_REQUEST['measurement_label']));
                }
        ...
        public function handle_ajax_upload() {            
            ...
            $files = array(); $post_files = array();
            $fileObj = new stdClass();
            $fileName = ""; $fileExt = "";
            
            $_session_id = uniqid();
            if (isset($_REQUEST['session'])) {
                $_session_id = uDrawUtil::sanitize_variables($_REQUEST['session']);
            }
            ...
            if (!is_null($_udraw_settings['udraw_price_matrix_allowed_file_types'])) {
                if (is_array($_udraw_settings['udraw_price_matrix_allowed_file_types'])) {
            ...
            if (isset($_REQUEST['filename']) && strlen($_REQUEST['filename']) > 0) {
                $custom_filename = $this->get_rand_num($upload_dir, uDrawUtil::sanitize_variables($_REQUEST['filename'])) . '_' . uDrawUtil::sanitize_variables($_REQUEST['filename']);
            }
                ...
                if ( !key_exists('error', $uploaded_files[0]) ) {
                    ...
                    if (isset($_REQUEST['greyscale'])) {
                        if (strtolower($fileExt) == 'pdf') {
                            ...
                            $data = array(
                                'pdfDocument' => str_replace('localhost', 'udraw-wordpress.ngrok.io', $fileObj->url),
