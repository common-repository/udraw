<?php
    ...
    if (current_user_can('edit_udraw_global_templates')) {
        if (isset($_GET['create_udraw_product'])) {
            ...
            $json = $uDrawUtil->get_web_contents(UDRAW_DRAW_SERVER_URL .'/api/templates/'. uDrawUtil::sanitize_variables($_GET['create_udraw_product']));
            ...
            if (strlen($udrawProduct[0]->AccessKey) > 1) {
                ...
                $results = $wpdb->get_row($sql, ARRAY_A);
                echo "<script>\n";
                echo "location.href = \"post-new.php?post_type=product&udraw_template_id=". uDrawUtil::esc_string($results["id"]) . "&udraw_action=new-product\";";
                echo "</script>\n";
            }
        }
    }
