...
<div class="wrap" id="manage-designs-page">
	...
	if (isset($_GET['udraw'])) {
		if ( ($_GET['udraw'] == 'delete') && (isset($_GET['id'])) ) {			
			...
			$wpdb->query("DELETE FROM $table_name WHERE id = '". esc_sql($_GET['id']) . "'");
			?>
        ...
        if ($_GET['udraw'] == 'duplicate') {
            if (uDraw::is_udraw_okay() == false) {
                if (count($udraw->get_udraw_templates()) >= 2) {
                    ...
                    $response = $udraw->duplicate_udraw_template(uDrawUtil::sanitize_variables($_GET['id']));
                }
            } else {
                $response = $udraw->duplicate_udraw_template(uDrawUtil::sanitize_variables($_GET['id']));
            }
...
<script type="text/javascript">
    ...
    function getFilter() {
        ...
        if (category != 'none') {
            document.location.href='?page=<?php echo uDrawUtil::esc_string($_REQUEST["page"])?>&filter='+category;
        } else {
            document.location.href='?page=<?php echo uDrawUtil::esc_string($_REQUEST["page"])?>';
        }
    }
