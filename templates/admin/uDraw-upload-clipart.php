    ...
    if (is_array($uploaded_files)) {
        for ($x = 0; $x < count($uploaded_files); $x++) {
            if ( !key_exists('error', $uploaded_files[$x]) ) {
                ...
                echo '<div class="error" style="padding: 10px; margin-left: 0px;"><span id="upload-error-span"><i class="fa fa-times-circle" style="color: red; transform: scale(1.5,1.5); margin-right: 5px;"></i>'. uDrawUtil::esc_string($uploaded_files[$x]['error']) .'</span></div>';
            }
        }
    }    
}
