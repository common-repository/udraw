...
if (isset($_GET['udraw_template_id']) && isset($_GET['udraw_action'])) {
    if ($_GET['udraw_action'] == "new-product") {
        $templateId = uDrawUtil::sanitize_variables($_GET['udraw_template_id']);                    
...
echo "<input type=\"hidden\" name=\"udraw_public_key\" value=\"". uDrawUtil::esc_string($udraw_public_key) . "\" />";
?>
...
<div id="udraw_product_data" class="panel woocommerce_options_panel">				
    <div class="options_group" id="udraw_template_id_form_group">
            ...
            <select id="udraw_template_id" name="udraw_template_id[]" multiple="multiple" data-placeholder="<?php _e('Select uDraw Template&hellip;', 'udraw'); ?>">
                ...
                if (count($templateId) == 0) {
                    ...
                    if (isset($_GET['udraw_template_id']) && isset($_GET['udraw_action'])) {
                        if ($_GET['udraw_action'] == "new-product") {
                            $templateId = array();
                            array_push($templateId, uDrawUtil::sanitize_variables($_GET['udraw_template_id'])); // pre-select template if linking as new product.
                        }
                    }
                }
                ...
                foreach($templates as $template) {
                    ...
                    if ($found_template_id) {                                        
                        echo '<option value="' . uDrawUtil::esc_string($template->id) . '" selected>' . esc_html($template->name . ' - ' . $template->design_width . '" x '. $template->design_height .'"') . '</option>';
                    } else {
                        echo '<option value="' . uDrawUtil::esc_string($template->id) . '">' . esc_html($template->name . ' - ' . $template->design_width . '" x '. $template->design_height .'"'). '</option>';
                    }
                }
        ...
        <p class="form-field" id="udraw_designer_skin_form_group" style="<?php echo uDrawUtil::esc_string($display); ?>">;
    ...
    <div class="options_group" id="udraw_pdf_template_id_form_group">
            ...
            <select id="udraw_block_template_id" name="udraw_block_template_id[]" multiple="multiple" data-placeholder="<?php _e('Select PDF Template&hellip;', 'udraw'); ?>" >
                        ...
                        if (strlen($block_template_id[0]) == 0) {
                            ...
                            if (isset($_GET['udraw_template_id']) && isset($_GET['udraw_action'])) {
                                if ($_GET['udraw_action'] == "new-block-product") {
                                    $block_template_id = array();
                                    array_push($block_template_id, uDrawUtil::sanitize_variables($_GET['udraw_template_id'])); // pre-select template if linking as new product.
                                }
                            }
                        }
                    }
                ...
                foreach($block_templates as $block_template) {
                    ...
                    if ($found_block_template_id) {                                        
                        echo '<option value="' . uDrawUtil::esc_string($block_template['UniqueID']) . '" selected>' . esc_html($block_template['ProductName']) . '</option>';
                    } else {
                        echo '<option value="' . uDrawUtil::esc_string($block_template['UniqueID']) . '">' . esc_html($block_template['ProductName']). '</option>';
                    }
        ...
        <p class="form-field" id="udraw_pdf_layout_form_group" style="<?php echo uDrawUtil::esc_string($style); ?>">;
            ...
            <select id="udraw_xmpie_template_id" name="udraw_xmpie_template_id[]" multiple="multiple" data-placeholder="<?php _e('Select XmPie Template&hellip;', 'udraw'); ?>" >
                ...
                if ($xmpie_template_id_length == 0) {
                    ...
                    if (isset($_GET['udraw_template_id']) && isset($_GET['udraw_action'])) {
                        if ($_GET['udraw_action'] == "new-xmpie-product") {
                            ...
                            array_push($xmpie_template_id, uDrawUtil::sanitize_variables($_GET['udraw_template_id'])); // pre-select template if linking as new product.
                        }
                    }
                }
                ...
                foreach($xmpie_templates as $xmpie_template) {
                    ...
                    if ($found_xmpie_template_id) {                                        
                        echo '<option value="' . uDrawUtil::esc_string($xmpie_template['ProductID']) . '" selected>' . esc_html($xmpie_template['ProductName']) . '</option>';
                    } else {
                        echo '<option value="' . uDrawUtil::esc_string($xmpie_template['ProductID']) . '">' . esc_html($xmpie_template['ProductName']). '</option>';
                    }
    ...
    <div class="options_group">
        ...
        <p class="form-field" id="udraw_disable_cart_preview_thumbnail_form">
            <label for="udraw_disable_cart_preview_thumbnail">Disable Cart Preview Thumbnail</label>
            <?php
                $disableCartPreviewThumbnail = get_post_meta($post->ID, '_udraw_disable_cart_preview_thumbnail', true);
            ?>                        
            <input type="checkbox" class="checkbox" name="udraw_disable_cart_preview_thumbnail" id="udraw_disable_cart_preview_thumbnail" value="yes" <?php if ($disableCartPreviewThumbnail == "yes") { echo "checked=\"checked\""; } ?> />
            <span class="description">Disabling this will not generate a preview of design/ uploaded file and will display the standard product thumbnail instead. </span>
        </p>
        <p class="form-field" id="max_files_allowed">
            ...
            ?> 
            <input type="number" class="number" name="max_files_allowed" id="max_files_allowed" style="width: 350px" value="<?php echo uDrawUtil::esc_string($maxUploadFiles) ?>"/>
            ...
            <select id="udraw_private_users_list" name="udraw_private_users_list[]" multiple="multiple" data-placeholder="<?php _e('Select a Customer&hellip;', 'udraw'); ?>">
                ...
                do {
                    ...
                    while (count($reversed) > 0) {
                        ...
                        echo printf('<option value="%s" %s>%s - %s</option>', uDrawUtil::esc_string($customer->ID), uDrawUtil::esc_string($selected), esc_html($customer->display_name), uDrawUtil::esc_string($customer->user_email));
                    }
                    $page++;
    ...
    <div class="options_group">
            ...
            <select id="udraw_price_matrix_list" name="udraw_price_matrix_list[]" multiple="multiple" data-placeholder="Select a Price matrix&hellip">
                ...
                foreach ($price_matrix_list as $price_matrix_item) {
                    if (gettype($selected_price_matrix) == 'array' && $selected_price_matrix[0] == $price_matrix_item->access_key) {
                        echo '<option value="' . uDrawUtil::esc_string($price_matrix_item->access_key) . '" selected>' . esc_html($price_matrix_item->name) . '</option>';
                    } else {
                        echo '<option value="' . uDrawUtil::esc_string($price_matrix_item->access_key) . '">' . esc_html($price_matrix_item->name) . '</option>';
                    }
                }
        ...
        <p class="form-field">
            ...
            ?>
            <input name="_udraw_preset_private_image_categories" id="_udraw_preset_private_image_categories" type="text" style="width: 350px" value="<?php echo uDrawUtil::esc_string($presetPrivateImageCategories) ?>" class="">
    ...
    jQuery(document).ready(function($) {
        ...
        if (isset($_GET['udraw_template_id']) && isset($_GET['udraw_action'])) {
            if ($_GET['udraw_action'] == "new-product") {
                ...
                $block_template_id = uDrawUtil::sanitize_variables($_GET['udraw_template_id']);
                ...
                $xmpie_template_id = uDrawUtil::sanitize_variables($_GET['udraw_template_id']);
