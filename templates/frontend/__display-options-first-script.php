...
<script>
    ...
    jQuery(document).ready(function () {
        ...
        <?php if (strlen($_cart_item_key) > 0) { ?>
            ...
            jQuery('input[name="udraw_product_cart_item_key"]').val('<?php echo uDrawUtil::esc_string($_cart_item_key); ?>');

            <?php if (isset($is_upload_product) || (isset($is_upload_product_update) && $is_upload_product_update)) { ?>
                jQuery('input[name="udraw_options_uploaded_files"]').val('<?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($cart_item['udraw_data']['udraw_options_uploaded_files']))); ?>');
                ...
                jQuery('input[name="udraw_options_converted_pdf"]').val('<?php echo uDrawUtil::esc_string($cart_item['udraw_data']['udraw_options_converted_pdf']); ?>');
        ...
        function _handle_file_upload_completed(e, data) {
            ...
            for (var x = 0; x < data.result.length; x++) {
                if (typeof data.result[x].error == 'string') {
                    console.log(data.result[x].error);
                    var _valid_extension_names = '';
                    <?php
                    if (isset($valid_extensions)) {
                        if (strlen($valid_extensions) > 0) {
                            echo '_valid_extension_names="'. uDrawUtil::esc_string($valid_extensions) .'";';
                        }
                    }
                    ?>
                ...
                if (uploadedFiles.length == 1) {
                    ...
                    if (extension.toLowerCase() == '.pdf') {
                        ...
                        jQuery('#convert-pdf-btn').click(function(){
                            ...
                            var activationKey = '<?php echo uDrawUtil::esc_string($_activation_key) ?>';
                            var file = url;
    ...
    <?php if ( ($_udraw_settings['show_customer_preview_before_adding_to_cart']) && (strlen($_cart_item_key) == 0) ) { ?>
        jQuery('#udraw-options-submit-form-btn').on('click', function (evt) {
            if (typeof required_field_validation == 'function') {
                ...
                if (validation) {
                    add_to_cart_after_required_input_validation(evt);
                } else {
    ...
    function add_to_cart_after_required_input_validation (evt) {
        evt.preventDefault();
            ...
            if (uploadedFiles.length > 0) {
                for (var i=0; i < uploadedFiles.length ; i++) {
                    if (uploadedFiles[i].url.endsWith('.pdf')) {
                        var iFrameSrc = '<?php echo uDrawUtil::esc_string(UDRAW_PLUGIN_URL) ?>' + '/assets/pdfjs/web/viewer.php?ps=no&file=' + uploadedFiles[i].url;
