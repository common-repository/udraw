 ...
 * @version 2.3.0
 */
error_log(print_r('I AM RUNNING v2...', true));
if ( ! defined( 'ABSPATH' ) ) {
    exit;
}
...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo uDrawUtil::esc_string($post->ID); ?>" data-product_variations="<?php echo uDrawUtil::esc_string( json_encode( $available_variations ) ) ?>">
    ...
    <table class="variations" cellspacing="0">
        ...
        <?php $loop = 0; foreach ( $attributes as $name => $options ) : $loop++; ?>
            <tr>
                ...
                <td class="value">
                <select class="variation_select" id="<?php echo uDrawUtil::esc_string( sanitize_title( $name ) ); ?>" name="attribute_<?php echo sanitize_title( $name ); ?>" data-attribute_name="attribute_<?php echo sanitize_title( $name ); ?>">
                    ...
                    if ( is_array( $options ) ) {
                        ...
                        if ( taxonomy_exists( $name ) ) {
                            ...
                            foreach ( $terms as $term ) {
                                ...
                                echo '<option value="' . uDrawUtil::esc_string( $term->slug ) . '" ' . selected( sanitize_title( $selected_value ), sanitize_title( $term->slug ), false ) . '>' . apply_filters( 'woocommerce_variation_option_name', $term->name ) . '</option>';
                            }
                        } else {
                            foreach ( $options as $option ) {
                                echo '<option value="' . uDrawUtil::esc_string(  $option  ) . '" ' . selected( uDrawUtil::esc_string( $selected_value ), uDrawUtil::esc_string( $option ), false ) . '>' . esc_html( apply_filters( 'woocommerce_variation_option_name', $option ) ) . '</option>';
                            }
                        }
                    }
                ?>
    ...
    <div class="single_variation_wrap" style="display:none;">
        ...
        <div class="variations_button">
            ...
            ?>
            <button type="submit" <?php echo htmlspecialchars($style, ENT_NOQUOTES); ?> id="udraw-options-submit-form-btn" class="single_add_to_cart_button button alt"><?php echo uDrawUtil::esc_string($product->single_add_to_cart_text()); ?></button>		
        </div>

        <input type="hidden" name="add-to-cart" value="<?php echo uDrawUtil::esc_string($product_id); ?>" />
        <input type="hidden" name="product_id" value="<?php echo uDrawUtil::esc_string( $post->ID ); ?>" />
...
<?php do_action( 'woocommerce_after_add_to_cart_form' ); ?>
    ...
    $is_disable_cart_preview_thumbnail = false;
    ...
    error_log(print_r('ALLOW UPLOAD ARTWORK', true));
    error_log(print_r($allowUploadArtwork, true));
