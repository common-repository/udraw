...
function generateCategoriesHtml($parent) {
    ...
    if ( $terms ){
        if ($parent == 0) {
            echo '<div id="udraw-cat-list-'. uDrawUtil::esc_string($parent) . '" class="udraw-cat-list">';
        } else {
            echo '<div id="udraw-cat-list-'. uDrawUtil::esc_string($parent) . '" class="udraw-cat-list" style="display:none;">';
        }
        echo '<ul>';
    ...
    ?>
        <li>                    
            <table>
                <tbody>
                    ...
                    <?php if (count($children) > 0) { ?>
                        <a href="#" onclick="shortcode_displaySubCategory('<?php echo uDrawUtil::esc_string($term->term_id); ?>');">
                    <?php } else { ?>
                        <a href="/?product_cat=<?php echo uDrawUtil::esc_string($term->slug) ?>">
                    <?php } ?>
                    <?php
            ...
            if ( $image ) {
                ...
                echo '<img class="alignnone size-thumbnail" src="'. uDrawUtil::esc_string(UDRAW_PLUGIN_URL) . '/assets/includes/no_image.jpg" alt="' . uDrawUtil::esc_string($term->name) . '" style="max-width:150px;max-height:150px;" />';
            }
                    ?>                                
                    </a>
                    </td>
                    ...
                    <td>
                        ...
                        <?php } else { ?>
                            <a href="/?product_cat=<?php echo uDrawUtil::esc_string($term->slug) ?>">
                        <?php } ?>
                        <h2 style="text-align: center; min-height:50px;"><?php echo uDrawUtil::esc_string($term->name); ?></h2>
                        </a>
                    </td>
                    </tr>
                </tbody>
