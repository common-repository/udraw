...
if (isset($_REQUEST['udraw_action']) && strlen($_REQUEST['udraw_action']) > 0) {
    $action = uDrawUtil::sanitize_variables($_REQUEST['udraw_action']);
    ?>
...
<form method="POST" action="?page=<?php echo uDrawUtil::esc_string($_REQUEST['page']); ?>">
            ...
            if (window.confirm('Delete template #' + template_id + '?')) {
                $.ajax({
                    ...
                    success: function (result) {
                        window.location.href = window.location.origin + window.location.pathname + '?page=<?php echo uDrawUtil::esc_string($_REQUEST['page']) ?>&udraw_action=deleted';
                    },
                    error: function (error) {
                        console.error(error);
