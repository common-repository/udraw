    ...
    class SVGDesigner_handler extends uDrawAjaxBase {
        ...
        public function save_svg ($output_path = '', $svg_string = '') {
            ...
            if (isset($_REQUEST['output_path'])) {
                $output_path = uDrawUtil::sanitize_variables($_REQUEST['output_path']);
            } else {
                $this->sendResponse(false);
            }
            if (isset($_REQUEST['data'])) {
                $svg_string = uDrawUtil::sanitize_variables($_REQUEST['data']);
            } else {
                $this->sendResponse(false);
            }
            if (isset($_REQUEST['preview_data'])) {
                $image_data = uDrawUtil::sanitize_variables($_REQUEST['preview_data']);
            }
        ...
        public function read_svg ($file_path = '') {
            ...
            if (isset($_REQUEST['design_file']) && strlen($_REQUEST['design_file'])) {
                $svg_file = uDrawUtil::sanitize_variables($_REQUEST['design_file']);
            } else {
                $this->sendResponse(false);
            }
			
			if (strpos($svg_file, "http") === 0) {
				$svg_file_url = $svg_file;
			} else {
				$svg_file_url = UDRAW_STORAGE_URL . $svg_file;
			}

            $svg_string = $uDrawUtil->get_web_contents($svg_file_url);
        ...
        public function upload_image($upload_path = '') {
            ...
            if (isset($_REQUEST['upload_path']) && strlen($_REQUEST['upload_path']) > 0) {
                $upload_path = uDrawUtil::sanitize_variables($_REQUEST['upload_path']);
            } else {
                $this->sendResponse(false);
            }
            ...
            if ($file_ext == 'pdf' || $file_ext == 'eps') {
                ...
                if ($file_ext != 'jpg' && $file_ext != 'jpeg' && $file_ext != 'png' && $file_ext != 'gif' && $file_ext != 'svg') {
                    ...
                    if ($file_ext == 'svg') {
                        ...
                        if ($isMalformed) {
                            $upload_url = str_replace(UDRAW_STORAGE_DIR, UDRAW_STORAGE_URL, $upload_dir);
                            $svg_file = $uDrawUtil->get_web_contents($upload_url . '/'. $new_file);
        ...
        public function uploaded_images ($path = '') {
            if (isset($_REQUEST['path'])) {
                $path = uDrawUtil::sanitize_variables($_REQUEST['path']);
            } else {
                $this->sendResponse(false);
            }
        ...
        public function download_image ($image_source = '', $image_name = '', $image_dir = '') {
            if (isset($_REQUEST['source']) && isset($_REQUEST['image_name']) && isset($_REQUEST['upload_path'])) {
                $image_source = uDrawUtil::sanitize_variables($_REQUEST['source']);
                $image_name = uDrawUtil::sanitize_variables($_REQUEST['image_name']);
                $path = uDrawUtil::sanitize_variables($_REQUEST['upload_path']);
            } else {
                $this->sendResponse(false);
            }
        ...
        public function export_image ($image_data = '', $export_path = '') {
            if (isset($_REQUEST['image_data']) && isset($_REQUEST['export_path'])) {
                $image_data = uDrawUtil::sanitize_variables($_REQUEST['image_data']);
                $export_path = uDrawUtil::sanitize_variables($_REQUEST['export_path']);
            } else {
                $this->sendResponse(false);
            }
        ...
        public function process_fonts () {
            $font_path = uDrawUtil::sanitize_variables($_REQUEST['font_path']);
        ...
        public function retrieve_instagram() {
            $path = (isset($_REQUEST['url'])) ? uDrawUtil::sanitize_variables($_REQUEST['url']) : '';
            if (strlen($path) === 0) {
        ...
        public function save_page () {
            ...
            if (isset($_REQUEST['data']) && isset($_REQUEST['current_file'])) {
                $svg_string = uDrawUtil::sanitize_variables($_REQUEST['data']);
                $current_file = uDrawUtil::sanitize_variables($_REQUEST['current_file']);
            } else {
                $this->sendResponse(false);
            }
            try {
                ...
                if (isset($_REQUEST['preview_data']) && isset($_REQUEST['preview_file'])) {
                    $preview_file = uDrawUtil::sanitize_variables($_REQUEST['preview_file']);
                    $image_data = uDrawUtil::sanitize_variables($_REQUEST['preview_data']);
                    if (strlen($image_data) > 0) {
        ...
        public function create_page () {
            if (isset($_REQUEST['page_list']) && isset($_REQUEST['session_id']) && isset($_REQUEST['output_path'])) {
                $page_list = uDrawUtil::sanitize_variables($_REQUEST['page_list']);
        ...
        public function convert_url_to_base64 () {
            ...
            if (isset($_REQUEST['url']) && strlen($_REQUEST['url']) > 0) {
                if (substr( $_REQUEST['url'], 0, 4 ) === "http") {
                    $response = $uDrawUtil->get_web_contents($_REQUEST['url']);
                    if ($response !== false) {
