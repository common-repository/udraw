...
<?php
    ...
    if (current_user_can('edit_udraw_global_templates')) {
        if (isset($_GET['create_udraw_svg_product'])) {
            $uDrawUtil = new uDrawUtil();
            $template_url = UDRAW_DRAW_SERVER_URL .'/api/svg/'. uDrawUtil::sanitize_variables($_GET['create_udraw_svg_product']);
            ...
            if ($design && strlen($design->AccessKey) > 1) {
                ...
                echo "<script>\n";
                echo "location.href = \"post-new.php?post_type=product&udraw_svg_template_id=". uDrawUtil::esc_string($results["ID"]) . "&udraw_svg_action=new-product\";";
                echo "</script>\n";
            }
        }
    }
