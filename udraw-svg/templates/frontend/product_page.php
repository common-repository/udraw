...
if (isset($_udraw_settings['udraw_price_matrix_allowed_file_types']) && is_array($_udraw_settings['udraw_price_matrix_allowed_file_types'])) {
    $valid_extensions = "";
    foreach($_udraw_settings['udraw_price_matrix_allowed_file_types'] as $key => $value) {
...
$cart_item_key = (isset($_REQUEST['cart_item_key'])) ? uDrawUtil::sanitize_variables($_REQUEST['cart_item_key']) : '';
?>
...
?>
    ...
    <br />
    <input type="file" name="files[]" data-udrawSVG="upload_template" class="hidden" accept="<?php echo uDrawUtil::esc_string($valid_extensions) ?>" />
            ...
            <tbody>
                ...
                if ($allow_upload ) {
                    for ($i = 0; $i < count($upload_pages); $i++) {
                        ?>
                        <tr>
                            <td><label><?php echo uDrawUtil::esc_string($upload_pages[$i]->label) ?></label></td>
                            <td><input type="file" id="<?php echo str_replace(' ', '_', $upload_pages[$i]->label); ?>" name="files[]" class="upload_artwork_page hidden" accept="<?php echo uDrawUtil::esc_string($valid_extensions) ?>">
                    ...
                    ?>
                        <td><input type="file" id="file_upload" name="files[]" class="upload_artwork_page hidden" accept="<?php echo uDrawUtil::esc_string($valid_extensions) ?>">
    ...
    <script>
        ...
        jQuery(document).ready(function($){
            ...
            $('input.upload_artwork_page').each(function(){
                ...
                var session_id = '<?php echo uDrawUtil::esc_string($session_id) ?>';
