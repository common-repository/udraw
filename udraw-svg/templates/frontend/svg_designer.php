...
if (isset($_REQUEST['udraw_access_key'])) {
    $udraw_access_key = uDrawUtil::sanitize_variables($_REQUEST['udraw_access_key']);
...
if ($SVG_product) {
    ...
    if (isset($_REQUEST['cart_item_key'])) {
        $cart_item_key = uDrawUtil::sanitize_variables($_REQUEST['cart_item_key']);
        ...
        if (isset($cart_item['udraw_SVG_data'])) {
			...
			if (strpos($design_file, "http") !== 0) {
				$svg_file_url = UDRAW_STORAGE_URL . $design_file;
			} 
    ...
    <form method="POST" action="" name="udraw_save_later_form" id="udraw_save_later">
        ...
        <input type="hidden" name="udraw_save_access_key" value="<?php echo uDrawUtil::esc_string($udraw_access_key); ?>" />
...
<script>
    ...
    function __load_settings() {
        ...
        RacadSVGDesigner.settings.design_file = '<?php echo uDrawUtil::esc_string($design_file); ?>';
        
        <?php if ($_udraw_settings['designer_enable_facebook_functions']) { ?>
            RacadSVGDesigner.settings.facebook_client_id = '<?php echo uDrawUtil::esc_string($_udraw_settings['designer_facebook_app_id']) ?>';
        <?php } ?>
        <?php if ($_udraw_settings['designer_enable_instagram_functions']) { ?>
            RacadSVGDesigner.settings.instagram_client_id = '<?php echo uDrawUtil::esc_string($_udraw_settings['designer_instagram_client_id'])?>';
        <?php } ?>
        <?php if ($_udraw_settings['designer_enable_google_functions']) { ?>
            RacadSVGDesigner.settings.google_api_key = '<?php echo uDrawUtil::esc_string($_udraw_settings['designer_google_api_key']) ?>';
            RacadSVGDesigner.settings.google_client_id = '<?php echo uDrawUtil::esc_string($_udraw_settings['designer_google_client_id']) ?>';
            RacadSVGDesigner.settings.google_photos_src = '<?php echo uDrawUtil::esc_string(UDRAW_SVG_IMAGE_URL) ?>Google_Photos_icon.svg',
        <?php } ?>
        <?php if(isset($session_id) && strlen($session_id) > 0) { ?>
            RacadSVGDesigner.settings.session_id = '<?php uDrawUtil::esc_string($session_id) ?>';
        <?php } ?>
        <?php if ($svg_settings['udraw_SVGDesigner_enable_dpi']) { ?>
            RacadSVGDesigner.settings.minimum_dpi_requirement = parseInt('<?php echo uDrawUtil::esc_string($svg_settings['udraw_SVGDesigner_minimum_dpi']) ?>');
        <?php } ?>
        <?php if ($svg_settings['udraw_SVGDesigner_enable_stock_images']) { ?>
            RacadSVGDesigner.settings.load_private_images = true;
            RacadSVGDesigner.settings.private_image_library_path = '<?php echo uDrawUtil::esc_string(UDRAW_CLIPART_URL) ?>';
        <?php } ?>
        <?php if (isset($display_layers) && $display_layers) { ?>
            RacadSVGDesigner.settings.display_layers = !parseInt('<?php echo uDrawUtil::esc_string($display_layers) ?>');
        <?php } ?>
    ...
    jQuery(document).ready(function($){
        window.use_edit_text_modal = !parseInt('<?php echo uDrawUtil::esc_string($svg_settings['udraw_SVGDesigner_tab_text_editor']) ?>');
        <?php if ($is_update) { ?>
            $('[data-udrawSVG="SVGDesigner"]').on('udraw_svg_designer_loaded', function(){
                $('input[name="udraw_SVG_product"]').val(true);
                $('input[name="udraw_SVG_product_data"]').val('<?php echo uDrawUtil::esc_string($design_file) ?>');
                $('input[name="udraw_SVG_product_preview"]').val(RacadSVGDesigner.settings.output_path + '<?php echo uDrawUtil::esc_string($design_preview) ?>');
                $('[name="udraw_svg_product_cart_item_key"]').val('<?php echo uDrawUtil::esc_string($cart_item_key) ?>');
                $('input[name="udraw_SVG_session_id"]').val('<?php echo uDrawUtil::esc_string($session_id) ?>');
            });
        <?php } ?>
        ...
        <?php if (strlen($udraw_access_key) > 0) { ?>
            $('[data-udrawSVG="SVGDesigner"]').on('udraw_svg_designer_loaded', function(){
                var design_string = '<?php echo uDrawUtil::esc_string($design); ?>';
        ...
        <?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($svg_settings['udraw_svg_js_hook']))); ?>
    });
</script>
<style>
    ...
    <?php if ($background_image_isset) { ?>
        [data-udrawsvg="SVGDesigner"] {
            background-image: url(<?php echo uDrawUtil::esc_string($selected_background_image_url) ?>);
    ...
    <?php } else { 
        ...
        [data-udrawsvg="SVGDesigner"] {
            background: <?php echo uDrawUtil::esc_string($selected_background_colour) ?>;
        }
        <?php 
        }
    } ?>
    [data-udrawSVG="SVGDesigner"] div.main_body > div.editing_tips_div {
        color: <?php echo uDrawUtil::esc_string($editing_tips_colour); ?>
    }
    ...
    <?php echo stripcslashes(htmlspecialchars_decode(uDrawUtil::esc_js($svg_settings['udraw_svg_css_hook']))); ?>
