...
try {
  ...
  while (!$done) {
    ...
    echo "doc status: " . uDrawUtil::esc_string($status_response->getStatus()) . "\n";
    switch ($status_response->getStatus()) {
      ...
      case "failed":
        echo "FALIED\n";
        echo uDrawUtil::esc_string($status_response);
        $done = true;
        break;
      default:
        sleep(1);
  ...
  echo uDrawUtil::esc_string($exception) . "\n";
  echo uDrawUtil::esc_string($exception->getMessage()) . "\n";
  echo uDrawUtil::esc_string($exception->getCode()) . "\n";
  echo uDrawUtil::esc_string($exception->getResponseBody()) . "\n";
}
